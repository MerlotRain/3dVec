...
RTransaction::RTransaction()
    : types(Generic), storage(NULL), transactionId(-1), transactionGroup(-1),
      undoable(true), failed(false), onlyChanges(true),
      recordAffectedObjects(true), allowAll(false), allowInvisible(false),
      spatialIndexDisabled(false), existingBlockDetectionDisabled(false),
      existingLayerDetectionDisabled(false),
      blockRecursionDetectionDisabled(false), keepHandles(false),
      keepChildren(false), undoing(false), redoing(false)
{
}
...
RTransaction::RTransaction(RStorage &storage)
    : types(Generic), storage(&storage), transactionId(-1),
      transactionGroup(-1), undoable(true), failed(false), onlyChanges(true),
      recordAffectedObjects(true), allowAll(false), allowInvisible(false),
      spatialIndexDisabled(false), existingBlockDetectionDisabled(false),
      existingLayerDetectionDisabled(false),
      blockRecursionDetectionDisabled(false), keepHandles(false),
...
RTransaction::RTransaction(
    ...
    //RTransaction* parent)
    : types(Generic), storage(&storage), transactionId(transactionId),
      transactionGroup(-1), text(text), affectedObjectIds(affectedObjectIds),
      propertyChanges(propertyChanges), undoable(true), failed(false),
      onlyChanges(true), recordAffectedObjects(true), allowAll(false),
      allowInvisible(false), spatialIndexDisabled(false),
      ...
      keepChildren(false), undoing(false), redoing(false)
{
    ...
    //affectedObjectIdsSet = affectedObjectIds;
}
 ...
 */
RTransaction::RTransaction(RStorage &storage, const QString &text,
                           bool undoable)
    //RTransaction* parent)
    : types(Generic), storage(&storage), transactionId(-1),
      transactionGroup(-1), text(text), undoable(undoable), failed(false),
      onlyChanges(true), recordAffectedObjects(true), allowAll(false),
      allowInvisible(false), spatialIndexDisabled(false),
      existingBlockDetectionDisabled(false),
      existingLayerDetectionDisabled(false),
...
void RTransaction::redo()
{
    ...
    for (int k = 0; k < affectedObjectIds.size(); ++k)
    {
        ...
        else
        {
            ...
            if (object.isNull())
            {
                qWarning("RTransaction::redo: object '%d' not found in storage",
                         objId);
                continue;
            }
            if (document != NULL)
            ...
            for (int i = 0; i < objectChanges.size(); ++i)
            {
                RPropertyTypeId propertyTypeId =
                        objectChanges.at(i).propertyTypeId;
                object->setProperty(propertyTypeId,
                                    objectChanges.at(i).newValue);
            }
...
void RTransaction::endCycle()
{
    ...
    for (int i = 0; i < affectedObjectIds.size(); i++)
    {
        ...
        {
            storage->setEntityParentId(
                    *entity, cloneIds.value(parentId, REntity::INVALID_ID));
        }
    }
    cloneIds.clear();
}
...
void RTransaction::commit()
{
    ...
    if (affectedObjectIds.size() > 0) { storage->saveTransaction(*this); }
...
void RTransaction::rollback() { storage->rollbackTransaction(); }
...
void RTransaction::updateAffectedBlockReferences()
{
    ...
    for (it = affectedBlockReferenceIds.begin();
         it != affectedBlockReferenceIds.end(); ++it)
    {
    ...
    if (hasBlock)
    {
        ...
        for (it = refs.begin(); it != refs.end(); ++it)
        {
            QSharedPointer<RBlockReferenceEntity> e =
                    storage->queryEntity(*it)
                            .dynamicCast<RBlockReferenceEntity>();
 ...
 */
bool RTransaction::addObject(
        QSharedPointer<RObject> object, bool useCurrentAttributes,
        bool forceNew, const QSet<RPropertyTypeId> &modifiedPropertyTypeIds)
{
    ...
    if (storageIsLinked && objectStorage != ls &&
        objectStorage != ls->getBackStorage())
    {
        qWarning() << "RTransaction::addObject: "
                      "Object is not in linked storage or back storage of "
                      "transaction.";
        fail();
        Q_ASSERT(false);
        return false;
    ...
    if (!entity.isNull() && entity->getId() != REntity::INVALID_ID)
    {
        // always clone:
        //mustClone = true;
        QSharedPointer<REntity> oldEntity =
                storage->queryEntityDirect(entity->getId());
        if (!oldEntity.isNull())
        {
            // object is entity and not new:
            if (oldEntity->cloneOnChange()) { mustClone = true; }
        }
        if (!mustClone)
        {
            if (entity->getType() == RS::EntityHatch &&
                modifiedPropertyTypeIds.isEmpty())
            {
    ...
    {
        if (object->mustAlwaysClone()) { mustClone = true; }
    }
    ...
    if (mustClone)
    {
        QSharedPointer<RObject> clone =
                QSharedPointer<RObject>(object->clone());
    ...
    if (!entity.isNull())
    {
        ...
        if (useCurrentAttributes ||
            entity->getLineweight() == RLineweight::WeightInvalid)
        {
            entity->setLineweight(doc->getCurrentLineweight());
        }
        if (useCurrentAttributes ||
            entity->getLinetypeId() == RLinetype::INVALID_ID)
        {
        ...
        {
            qWarning() << "RTransaction::addObject: entity not editable "
        ...
        Q_ASSERT_X(!doc->queryLayerDirect(entity->getLayerId()).isNull(),
                   "RTransaction::addObject", "layer of entity is NULL");
    }
    ...
    if (object->getType() == RS::ObjectBlock)
    {
        ...
        if (!block.isNull())
        {
            ...
            {
                QSharedPointer<RBlock> existingBlock =
                        block->getDocument()->queryBlock(block->getName());
    ...
    if (object->getType() == RS::ObjectLayer)
    {
            ...
            object->getId() == RObject::INVALID_ID)
        {
            ...
            if (!layer.isNull())
            {
                QSharedPointer<RLayer> existingLayer =
                        layer->getDocument()->queryLayer(layer->getName());
    ...
    {
        if (!existingLinetypeDetectionDisabled &&
            object->getId() == RObject::INVALID_ID)
        {
            QSharedPointer<RLinetype> linetype =
                    object.dynamicCast<RLinetype>();
            if (!linetype.isNull())
            {
                QSharedPointer<RLinetype> existingLinetype =
                        linetype->getDocument()->queryLinetype(
                                linetype->getName());
    ...
    if (object->getId() != RObject::INVALID_ID)
    {
        ...
        else
        {
            ...
            for (it = propertyTypeIds.begin(); it != propertyTypeIds.end();
                 ++it)
            {
        ...
        for (it = propertyTypeIds.begin(); it != propertyTypeIds.end(); ++it)
        {
            ...
            if (newProperty.second.isRedundant() ||
                oldProperty.second.isRedundant())
            {
                continue;
            }
            ...
            if (newProperty.second.affectsOtherProperties() ||
                oldProperty.second.affectsOtherProperties())
            {
                onlyChanges = false;
            }
            ...
            objectHasChanged |= addPropertyChange(
                    object->getId(),
                    RPropertyChange(pid, oldProperty.first, newProperty.first));
        }
        ...
        if (objectHasChanged)
        {
            ...
            {
                QSharedPointer<REntity> oldEntity =
                        oldObject.dynamicCast<REntity>();
                if (!spatialIndexDisabled && !oldEntity.isNull())
                {
                    QSharedPointer<RBlockReferenceEntity> blockRef =
                            oldEntity.dynamicCast<RBlockReferenceEntity>();
                    if (blockRef.isNull() ||
                        blockRef->getReferencedBlockId() != RObject::INVALID_ID)
                    {
        ...
        (objectIsBlock && storageIsLinked))
    {
        ...
        // new object:
        if (object->getId() == RObject::INVALID_ID) { onlyChanges = false; }
            ...
            if (!storageIsLinked)
            {
                ...
                if (!spatialIndexDisabled && !entity.isNull())
                {
                    ...
                    if (entity->getBlockId() != storage->getCurrentBlockId())
                    {
                        ...
                        affectedBlockReferenceIds.unite(
                                storage->queryBlockReferences(
                                        entity->getBlockId()));
                    }
                }
            }

            if (!objectHasChanged) { statusChanges.insert(object->getId()); }
        }
    }
    if (!ret) { fail(); }
...
{
    if (!recordAffectedObjects) { return; }

    if (object.isNull()) { return; }

    //if (affectedObjectIds.contains(object->getId())) {
    if (affectedObjectIdsSet.contains(object->getId())) { return; }
    ...
    if (!entity.isNull())
    {
        ...
        if (!affectedObjectIdsSet.contains(entity->getBlockId()))
        {
            ...
            {
                QSet<REntity::Id> affectedBlockRefIds =
                        storage->queryBlockReferences(entity->getBlockId());
...
void RTransaction::deleteObject(QSharedPointer<RObject> object, bool force)
{
    ...
    {
        failed = true;
        qWarning() << "RTransaction::deleteObject: trying to delete protected "
                      "object";
        return;
    }
    ...
    if (!layer.isNull())
    {
        ...
        while (it.hasNext()) { deleteObject(it.next(), true); }
    ...
    if (!entity.isNull())
    {
        ...
        {
            qWarning() << "RTransaction::deleteObject: entity not editable "
                          "(locked or hidden layer)";
            fail();
            return;
        }
...
{
    if (cloneIds.contains(oldId)) { return cloneIds[oldId]; }
...
{
    if (!propertyChanges.contains(id)) { return QList<RPropertyChange>(); }
