...
public:
    ...
    static RS::EntityType getRtti() { return RS::ObjectLinetype; }

    virtual RS::EntityType getType() const { return RS::ObjectLinetype; }

    virtual RLinetype *clone() const { return nullptr; }

    QString getName() const { return pattern.getName(); }

    void setName(const QString &n) { pattern.setName(n); }

    bool isMetric() const { return pattern.isMetric(); }

    void setMetric(bool on) { pattern.setMetric(on); }

    QString getDescription() const { return pattern.getDescription(); }

    void setDescription(const QString &d) { pattern.setDescription(d); }

    QString getLabel() const { return pattern.getLabel(); }

    QString getPatternString() const { return pattern.getPatternString(); }

    void setPatternString(const QString &s) { pattern.setPatternString(s); }
