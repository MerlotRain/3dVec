...
public:
    ...
    void setDocument(RDocument *document) { this->document = document; }
     ...
     */
    RObject::Id getClosestSubEntityId(const RVector &pos, double range,
                                      bool ignoreComplex) const
    {
        RObject::Id ret;
        QSharedPointer<RShape> shape =
                getClosestShape(pos, range, ignoreComplex, &ret);
        return ret;
    }
    ...
    virtual RShape *castToShape() { return NULL; }
     ...
     */
    virtual bool isUpdatesEnabled() const { return updatesEnabled; }

    /**
     * Enables / disables cache updates.
     */
    void setUpdatesEnabled(bool on) { updatesEnabled = on; }

    /**
     * Reimplement to force clone on change in transactions.
     */
    virtual bool cloneOnChange() const { return false; }

    /**
     * \return True if the entity is currently selected. This can for example
     *      influence the color in which the entity is exported.
     */
    virtual bool isSelected() const { return selectionStatus; }

    /**
     * Selects or deselects this entity.
     */
    virtual void setSelected(bool on) { selectionStatus = on; }
     ...
     */
    virtual void setDrawOrder(int drawOrder) { this->drawOrder = drawOrder; }

    virtual void setLayerId(RLayer::Id layerId) { this->layerId = layerId; }

    RLayer::Id getLayerId() const { return layerId; }
    ...
    virtual RLinetype::Id
    getLinetypeId(bool resolve, const QStack<REntity *> &blockRefStack) const;
    ...
    virtual double getLinetypeScale() const { return linetypeScale; }

    virtual RLineweight::Lineweight getLineweight() const { return lineweight; }

    virtual RLineweight::Lineweight
    ...
    void setColor(const RColor &color) { this->color = color; }

    virtual RColor getColor() const { return color; }
     ...
     *      of the entity.
     */
    virtual QList<RRefPoint> getReferencePoints(
    ...
    virtual bool
    isOnEntity(const RVector &point, bool limited = true,
    ...
    moveReferencePoint(const RVector &referencePoint,
                       ...
                       Qt::KeyboardModifiers modifiers = Qt::NoModifier) = 0;
