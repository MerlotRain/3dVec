...
RTextBasedEntity::RTextBasedEntity(RDocument *document) : REntity(document) {}
...
void RTextBasedEntity::init()
{
    ...
    RTextBasedEntity::PropertyLinetypeScale.generateId(
            RTextBasedEntity::getRtti(), REntity::PropertyLinetypeScale);
    RTextBasedEntity::PropertyLineweight.generateId(
            RTextBasedEntity::getRtti(), REntity::PropertyLineweight);
    RTextBasedEntity::PropertyColor.generateId(RTextBasedEntity::getRtti(),
                                               REntity::PropertyColor);
    RTextBasedEntity::PropertyDisplayedColor.generateId(
            RTextBasedEntity::getRtti(), REntity::PropertyDisplayedColor);
    RTextBasedEntity::PropertyDrawOrder.generateId(RTextBasedEntity::getRtti(),
                                                   REntity::PropertyDrawOrder);

    RTextBasedEntity::PropertySimple.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Simple"));
    RTextBasedEntity::PropertyPositionX.generateId(
            RTextBasedEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Text Position"),
            QT_TRANSLATE_NOOP("REntity", "X"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyPositionY.generateId(
            RTextBasedEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Text Position"),
            QT_TRANSLATE_NOOP("REntity", "Y"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyPositionZ.generateId(
            RTextBasedEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Text Position"),
            QT_TRANSLATE_NOOP("REntity", "Z"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyText.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Text"));
    RTextBasedEntity::PropertyPlainText.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Plain Text"));
    RTextBasedEntity::PropertyFontName.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Font Name"));
    RTextBasedEntity::PropertyHeight.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Text Height"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyWidth.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Text Width"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyAngle.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Text Angle"), false,
            RPropertyAttributes::Geometry);
    RTextBasedEntity::PropertyXScale.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "X Scale"));
    RTextBasedEntity::PropertyBold.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Bold"));
    RTextBasedEntity::PropertyItalic.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Italic"));
    RTextBasedEntity::PropertyLineSpacingFactor.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Line Spacing"));
    RTextBasedEntity::PropertyHAlign.generateId(
            RTextBasedEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Alignment"),
            QT_TRANSLATE_NOOP("REntity", "Horizontal"));
    RTextBasedEntity::PropertyVAlign.generateId(
            RTextBasedEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Alignment"),
            QT_TRANSLATE_NOOP("REntity", "Vertical"));
    RTextBasedEntity::PropertyBackward.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Backward"));
    RTextBasedEntity::PropertyUpsideDown.generateId(
            RTextBasedEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Upside Down"));
}
                                   ...
                                   RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMember(getData().alignmentPoint.x, value,
                                    PropertyPositionX == propertyTypeId);
    ret = ret || RObject::setMember(getData().alignmentPoint.y, value,
                                    PropertyPositionY == propertyTypeId);
    ret = ret || RObject::setMember(getData().alignmentPoint.z, value,
                                    PropertyPositionZ == propertyTypeId);
    ret = ret || RObject::setMember(getData().text, value,
                                    PropertyText == propertyTypeId);
    ret = ret || RObject::setMember(getData().fontName, value,
                                    PropertyFontName == propertyTypeId);
    ret = ret || RObject::setMember(getData().textHeight, value,
                                    PropertyHeight == propertyTypeId);
    ret = ret || RObject::setMember(getData().textWidth, value,
                                    PropertyWidth == propertyTypeId);
    ret = ret || RObject::setMember(getData().angle, value,
    ...
    ret = ret || RObject::setMember((int &) getData().horizontalAlignment,
                                    value.value<int>(),
                                    PropertyHAlign == propertyTypeId);
    ret = ret || RObject::setMember((int &) getData().verticalAlignment,
                                    value.value<int>(),
                                    PropertyVAlign == propertyTypeId);
    ...
    if (PropertyLineSpacingFactor == propertyTypeId)
    {
        ...
        {
            ret = ret ||
                  RObject::setMember(getData().lineSpacingFactor, value, true);
        }
    }
...
RTextBasedEntity::getProperty(RPropertyTypeId &propertyTypeId,
                              ...
                              bool showOnRequest)
{
    ...
    {
        return qMakePair(QVariant(getData().isSimple()),
                         RPropertyAttributes(
                                 RPropertyAttributes::AffectsOtherProperties));
    }
    else if (propertyTypeId == PropertyPositionX)
    {
        return qMakePair(QVariant(getData().alignmentPoint.x),
                         RPropertyAttributes());
    }
    else if (propertyTypeId == PropertyPositionY)
    {
        return qMakePair(QVariant(getData().alignmentPoint.y),
                         RPropertyAttributes());
    }
    else if (propertyTypeId == PropertyPositionZ)
    {
        return qMakePair(QVariant(getData().alignmentPoint.z),
                         RPropertyAttributes());
    }
    else if (propertyTypeId == PropertyText)
    {
        return qMakePair(QVariant(getData().text),
                         RPropertyAttributes(RPropertyAttributes::RichText |
                                             RPropertyAttributes::Label));
    }
    ...
    {
        return qMakePair(QVariant(getData().lineSpacingFactor),
                         RPropertyAttributes(RPropertyAttributes::UnitLess));
    }
    else if (propertyTypeId == PropertyHAlign)
    {
        ...
        return qMakePair(QVariant(getData().horizontalAlignment),
                         RPropertyAttributes());
    }
    else if (propertyTypeId == PropertyVAlign)
    {
        ...
        return qMakePair(QVariant(getData().verticalAlignment),
                         RPropertyAttributes());
    }
    else if (propertyTypeId == PropertyBackward)
    {
        return qMakePair(QVariant(getData().isBackward()),
                         RPropertyAttributes(
                                 isSimple() ? RPropertyAttributes::NoOptions
                                            : RPropertyAttributes::Invisible));
    }
    else if (propertyTypeId == PropertyUpsideDown)
    {
        return qMakePair(QVariant(getData().isUpsideDown()),
                         RPropertyAttributes(
                                 isSimple() ? RPropertyAttributes::NoOptions
                                            : RPropertyAttributes::Invisible));
    }
...
RTextBasedEntity::scaleNonUniform(const RVector &scaleFactors,
