...
REntityData::REntityData(RDocument *document)
    : document(document), updatesEnabled(true), selectionStatus(false),
      selectionStatusWorkingSet(false), autoUpdatesBlocked(false),
      drawOrder(RMININT), layerId(RLayer::INVALID_ID),
      blockId(RBlock::INVALID_ID), parentId(REntity::INVALID_ID),
      linetypeId(RLinetype::INVALID_ID), linetypeScale(1.0),
      lineweight(RLineweight::WeightInvalid), color()
{
...
// TODO: fix
double
REntityData::getLineweightInUnits(const QStack<REntity *> &blockRefStack) const
{
...
REntityData::getLineweight(bool resolve,
                           const QStack<REntity *> &blockRefStack) const
{

    if (!resolve) { return getLineweight(); }
        ...
        
        {
                ...
                blockRefStack.top()->getType() != RS::EntityViewport)
            {
                ...
                if (l->getName() == "0")
                {
                    if (!blockRefStack.isEmpty())
                    {
                        lw = blockRefStack.top()->getLineweight(true,
                                                                blockRefStack);
                    }
                }
            }
...
RLinetype::Id
REntityData::getLinetypeId(bool resolve,
                           const QStack<REntity *> &blockRefStack) const
{

    if (!resolve) { return getLinetypeId(); }
            ...
            
            {
                    ...
                    blockRefStack.top()->getType() != RS::EntityViewport)
                {
                    ...
                    if (l->getName() == "0")
                    {
                        ...
                        return blockRefStack.top()->getLinetypeId(
                                true, blockRefStack);
                    }
                }
            }
...
{
    if (document == NULL) { return RLinetypePattern(); }
 ...
 */
double REntityData::getDistanceTo(const RVector &point, bool limited,
                                     ...
                                     bool copyBlockId)
{
    ...
    {
        qWarning("REntityData::copyAttributesFrom: source entity not from same "
                 "document");
        return;
    }

    setLayerId(entityData.getLayerId());
    if (copyBlockId) { setBlockId(entityData.getBlockId()); }
...
RVector REntityData::getPointOnEntity() const
{
    ...
    if (s == NULL) { return RVector::invalid; }
 ...
 */
QList<RVector> REntityData::getEndPoints(const RBox &queryBox,
                                         QList<RObject::Id> *subEntityIds) const
{
    QList<RVector> ret;
 ...
 */
QList<RVector>
REntityData::getMiddlePoints(const RBox &queryBox,
                             QList<RObject::Id> *subEntityIds) const
{
    QList<RVector> ret;
    QList<QSharedPointer<RShape>> shapes;
 ...
 */
QList<RVector>
REntityData::getCenterPoints(const RBox &queryBox,
 ...
 */
QList<RVector>
REntityData::getPointsWithDistanceToEnd(double distance, int from,
                                        const RBox &queryBox,
                                        QList<RObject::Id> *subEntityIds) const
{
    QList<RVector> ret;
    ...
    if (subEntityIds != NULL)
    {
        ...
        {
            QList<RVector> pts =
                    shapes[i]->getPointsWithDistanceToEnd(distance, from);
            for (int k = 0; k < pts.length(); k++)
            {
                ret.append(pts[k]);
                if (i < entityIds.length())
 ...
 *        points on entity.
 */
RVector REntityData::getClosestPointOnEntity(const RVector &point, double range,
                                             bool limited,
                                             REntity::Id *subEntityId) const
{
    ...
    QList<REntity::Id> entityIds;
    QList<QSharedPointer<RShape>> shapes =
            getShapes(RBox(), true, false, &entityIds);
    for (int i = 0; i < shapes.size(); i++)
    {
 ...
 *   of an arc).
 */
RVector REntityData::getVectorTo(const RVector &point, bool limited,
                                 double strictRange) const
{
    RVector ret = RVector::invalid;
    QList<QSharedPointer<RShape>> shapes = getShapes();
 ...
 */
bool REntityData::isOnEntity(const RVector &point, bool limited,
                             double tolerance) const
{
    QList<QSharedPointer<RShape>> shapes = getShapes();
    for (int i = 0; i < shapes.size(); i++)
    {
        if (shapes.at(i)->isOnShape(point, limited, tolerance)) { return true; }
    }
    return false;
}
...
bool REntityData::intersectsWith(const RShape &shape) const
{
    ...
    for (int i = 0; i < shapes.size(); i++)
    {
        ...
        if (shapes.at(i)->intersectsWith(shape)) { return true; }
    }
    return false;
}
...
QList<RVector> REntityData::getIntersectionPoints(
        const REntityData &other, bool limited, bool same, const RBox &queryBox,
        bool ignoreComplex,
...
//    if (RMouseEvent::hasMouseMoved()) { return QList<RVector>(); }
    ...
    QList<QSharedPointer<RShape>> shapes2 =
            other.getShapes(queryBox, ignoreComplex, true, &entityIds2);
    ...
    for (int i = 0; i < shapes1.size(); i++)
    {
        ...
        for (int k = kStart; k < shapes2.size(); k++)
        {
           // if (RMouseEvent::hasMouseMoved()) { return QList<RVector>(); }
            ...
            QList<RVector> ips = shapes1.at(i)->getIntersectionPoints(
                    *shapes2.at(k), limited, false);
            ret.append(ips);
            ...
            if (entityIds != NULL)
            {
                ...
                if (i < entityIds1.length()) { id1 = entityIds1.at(i); }
                if (k < entityIds2.length()) { id2 = entityIds2.at(k); }
                // make sure the indices in ret and entityIds match, insert duplicates if necessary:
                for (int c = 0; c < ips.length(); c++)
                {
                    entityIds->append(
                            QPair<RObject::Id, RObject::Id>(id1, id2));
                }
            }
        }
    }
...
REntityData::getClosestShape(const RVector &pos, double range,
                             bool ignoreComplex, REntity::Id *subEntityId) const
{
    ...
    for (int i = 0; i < shapes.length(); i++)
    {
        ...
        QSharedPointer<RExplodable> explodable =
                shape.dynamicCast<RExplodable>();
        ...
        else { subShapes = explodable->getExploded(); }
...
void REntityData::setAutoUpdatesBlocked(bool on) { autoUpdatesBlocked = on; }
