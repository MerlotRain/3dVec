...
QMap<QPair<RS::EntityType, RPropertyAttributes::Option>, QSet<RPropertyTypeId>>
        RPropertyTypeId::propertyTypeByObjectOptionMap;
...
RPropertyTypeId::RPropertyTypeId(const QString &customPropertyTitle,
    ...
    : id(-1), options(false), customPropertyTitle(customPropertyTitle),
...
RPropertyTypeId::RPropertyTypeId(long int id) : id(id), options(false) {}
...
void RPropertyTypeId::generateId(RS::EntityType type, const QString &groupTitle,
                                 const QString &title, bool forceNew,
                                 RPropertyAttributes::Options options)
{
    ...
    {
        qWarning()
                << "RPropertyTypeId::generateId: property already initialized: "
                << type << ":" << groupTitle << ":" << title;
        return;
    }
        ...
        for (int i = 0; i < cachedOptionList.length(); i++)
        {
            if (this->options.testFlag(cachedOptionList[i]))
            {
                propertyTypeByObjectOptionMap
                        [QPair<RS::EntityType, RPropertyAttributes::Option>(
                                 type, cachedOptionList[i])]
                                .insert(*this);
            }
        }
    }
...
QString RPropertyTypeId::getPropertyGroupTitle() const
{
    ...
    else { return idToTitleMap[id].first; }
}
...
{
    if (isCustom()) { return customPropertyName; }
    else { return idToTitleMap[id].second; }
}
 ...
 */
long int RPropertyTypeId::getId() const { return id; }

void RPropertyTypeId::setId(long int id) { this->id = id; }
 ...
 */
bool RPropertyTypeId::hasPropertyType(RS::EntityType type,
                                      RPropertyTypeId propertyTypeId)
{

    if (!propertyTypeByObjectMap.contains(type)) { return false; }
...
RPropertyTypeId::getPropertyTypeIds(RS::EntityType type,
                                    RPropertyAttributes::Option option)
{
...
{
    if (id != INVALID_ID) { return id == other.id; }
    else
    {
        return id == other.id &&
               customPropertyName == other.customPropertyName &&
...
bool RPropertyTypeId::operator<(const RPropertyTypeId &other) const
{
    if (id != INVALID_ID && other.id != INVALID_ID) { return id < other.id; }
    else
    {
...
uint qHash(const RPropertyTypeId &propertyTypeId)
{
    ...
    {
        return qHash(propertyTypeId.getCustomPropertyTitle() + "///" +
