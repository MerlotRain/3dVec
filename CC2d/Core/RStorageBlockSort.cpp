...
bool RStorageBlockSort::operator()(RBlock::Id a, RBlock::Id b) const
{
    ...
    if (blockA.isNull() || blockB.isNull()) { return false; }

    // model space always first:
    if (blockA->getName() == RBlock::modelSpaceName) { return true; }
    if (blockB->getName() == RBlock::modelSpaceName) { return false; }
    ...
    if (blockTitleA.startsWith("*")) { blockTitleA = blockTitleA.mid(1); }
    if (blockTitleB.startsWith("*")) { blockTitleB = blockTitleB.mid(1); }
