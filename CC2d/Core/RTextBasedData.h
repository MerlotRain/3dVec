...
public:
    ...
    RTextBasedData(const RVector &position, const RVector &alignmentPoint,
                   double textHeight, double textWidth,
                   RS::VAlign verticalAlignment, RS::HAlign horizontalAlignment,
    ...
    static RS::EntityType getRtti() { return RS::EntityTextBased; }

    virtual RS::EntityType getType() const { return RS::EntityTextBased; }
    ...
    virtual bool isValid() const { return !text.isNull(); }
    ...
    getEndPoints(const RBox &queryBox = RDEFAULT_RBOX) const
    {
        Q_UNUSED(queryBox);
        return QList<RVector>();
    }
    virtual QList<RVector>
    getMiddlePoints(const RBox &queryBox = RDEFAULT_RBOX) const
    {
        Q_UNUSED(queryBox);
        return QList<RVector>();
    }
    virtual QList<RVector>
    getCenterPoints(const RBox &queryBox = RDEFAULT_RBOX) const
    {
        Q_UNUSED(queryBox);
        return QList<RVector>();
    }
    ...
    void setPosition(const RVector &position) { this->position = position; }

    /**
     * \return Alignment point of the reference point of the text.
     * This is usually the same as the position.
     */
    RVector getAlignmentPoint() const { return alignmentPoint; }
    ...
    QString getFontFile() const { return fontFile; }

    void setFontFile(const QString &fontFile) { this->fontFile = fontFile; }

    bool isBold() const { return getFlag(Bold); }
    ...
    bool isItalic() const { return getFlag(Italic); }
    ...
    double getTextHeight() const { return textHeight; }
     ...
     */
    double getTextWidth() const { return textWidth; }
    ...
    RS::VAlign getVAlign() const { return this->verticalAlignment; }
    ...
    RS::HAlign getHAlign() const { return this->horizontalAlignment; }
    ...
    bool isSimple() const { return getFlag(Simple); }
    ...
    bool isBackward() const { return isSimple() && getFlag(Backward); }
    ...
    bool isUpsideDown() const { return isSimple() && getFlag(UpsideDown); }
    ...
    bool isDimensionLabel() const { return getFlag(DimensionLabel); }
    ...
    void setHighlighted(bool on) { setFlag(Highlighted, on); }

    bool isHighlighted() const { return getFlag(Highlighted); }
    ...
    virtual void update() const { update(true); }
    ...
    virtual RVector getClosestPointOnEntity(const RVector &point, double range,
                                            bool limited) const;
    ...
    static QString toRichText(const QString &escapedText, const QFont &mainFont,
                              double fontHeightFactor = 1.0);
