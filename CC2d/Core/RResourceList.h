...
public:
     ...
     */
    QStringList getNames() { return resMap.keys(); }
    ...
    QString getSubName(const QString &resName, int rec = 0)
    {
        ...
        if (RS::mapContainsCaseInsensitive(resSubstitutionMap, resName))
        {
            // substitution found:
            QString subName =
                    RS::mapValueCaseInsensitive(resSubstitutionMap, resName);
            if (subName.compare(resName, Qt::CaseInsensitive) == 0 || rec > 16)
            {
                qWarning() << "recursive resource substitution:" << resName
                           << "->" << subName;
    ...
    T *get(const QString &resName, bool substitute = true)
    {
        ...
        if (substitute) { resNameSub = getSubName(resName); }
