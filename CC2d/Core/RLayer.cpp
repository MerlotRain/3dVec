...
RLayer::RLayer()
    : RObject(), flags(RLayer::Plottable | RLayer::Snappable),
      linetypeId(RLinetype::INVALID_ID), lineweight(RLineweight::WeightInvalid)
{

    //RDebug::incCounter("RLayer");
}

RLayer::RLayer(RDocument *document, const QString &name, bool frozen,
               bool locked, const RColor &color, RLinetype::Id linetype,
               RLineweight::Lineweight lineweight, bool off)
    :

      RObject(document), name(name.trimmed()), color(color),
      linetypeId(linetype), lineweight(lineweight)
{
...
RLayer::RLayer(const RLayer &other)
    : RObject(other), name(other.name), flags(other.flags), color(other.color),
      linetypeId(other.linetypeId), lineweight(other.lineweight)
{
...
void RLayer::init()
{
    ...
    RLayer::PropertyProtected.generateId(RLayer::getRtti(),
                                         RObject::PropertyProtected);
    RLayer::PropertySelected.generateId(RLayer::getRtti(),
                                        RObject::PropertySelected);

    RLayer::PropertyName.generateId(RLayer::getRtti(), "",
                                    QT_TRANSLATE_NOOP("REntity", "Name"));
    RLayer::PropertyOff.generateId(RLayer::getRtti(), "",
                                   QT_TRANSLATE_NOOP("REntity", "Off"));
    RLayer::PropertyFrozen.generateId(RLayer::getRtti(), "",
                                      QT_TRANSLATE_NOOP("REntity", "Frozen"));
    RLayer::PropertyLocked.generateId(RLayer::getRtti(), "",
                                      QT_TRANSLATE_NOOP("REntity", "Locked"));
    RLayer::PropertyCollapsed.generateId(
            RLayer::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Collapsed"));
    RLayer::PropertyPlottable.generateId(
            RLayer::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Plottable"));
    RLayer::PropertySnappable.generateId(
            RLayer::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Snappable"));
    RLayer::PropertyOffIsFreeze.generateId(
            RLayer::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Off is Freeze"));
    RLayer::PropertyColor.generateId(RLayer::getRtti(), "",
                                     QT_TRANSLATE_NOOP("REntity", "Color"));
    RLayer::PropertyLinetype.generateId(
            RLayer::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Linetype"));
    RLayer::PropertyLineweight.generateId(
            RLayer::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Lineweight"));
}

RLayer *RLayer::clone() const { return new RLayer(*this); }

RLayer::Id RLayer::getParentLayerId() const
{
    QString parentLayerName = getParentLayerName();
    if (parentLayerName.isEmpty()) { return RLayer::INVALID_ID; }

    const RDocument *doc = getDocument();
    if (doc == NULL) { return RLayer::INVALID_ID; }
...
bool RLayer::setProperty(RPropertyTypeId propertyTypeId, const QVariant &value,
                         RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMemberFlag(RLayer::Off, value,
                                        PropertyOff == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::Frozen, value,
                                        PropertyFrozen == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::Locked, value,
                                        PropertyLocked == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::Collapsed, value,
                                        PropertyCollapsed == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::Plottable, value,
                                        PropertyPlottable == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::Snappable, value,
                                        PropertySnappable == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RLayer::OffIsFreeze, value,
                                        PropertyOffIsFreeze == propertyTypeId);
    ret = ret ||
          RObject::setMember(color, value, PropertyColor == propertyTypeId);
...
RLayer::getProperty(RPropertyTypeId &propertyTypeId, bool humanReadable,
                    bool noAttributes, bool showOnRequest)
{
    ...
    {
        return qMakePair(QVariant(isOffIsFreeze()),
    ...
    if (propertyTypeId == PropertyLinetype)
    {
        ...
        else { return qMakePair(QVariant(linetypeId), RPropertyAttributes()); }
    }
