...
RBlockReferenceData::RBlockReferenceData()
    : referencedBlockId(RBlock::INVALID_ID), rotation(0), columnCount(1),
      rowCount(1), columnSpacing(0), rowSpacing(0), visualPropertiesScale(1.0)
{
}
...
RBlockReferenceData::RBlockReferenceData(RBlock::Id referencedBlockId,
                                         ...
                                         double rotation, int columnCount,
                                         int rowCount, double columnSpacing,
                                         double rowSpacing,
                                         double visualPropertiesScale)
    : referencedBlockId(referencedBlockId), position(position),
      scaleFactors(scaleFactors), rotation(rotation), columnCount(columnCount),
      rowCount(rowCount), columnSpacing(columnSpacing), rowSpacing(rowSpacing),
...
RVector RBlockReferenceData::getVectorTo(const RVector &point, bool limited,
                                         double strictRange) const
{
    Q_UNUSED(strictRange)

    if (document == NULL) { return RVector::invalid; }
    ...
    if (recursionDepth++ > 16)
    {
        ...
        qWarning() << "RBlockReferenceData::getVectorTo: "
                   << "maximum recursion depth reached: block: "
                   << getBlockName();
...
double RBlockReferenceData::getDistanceTo(const RVector &point, bool limited,
                                          double range, bool draft,
                                          double strictRange) const
{

    if (document == NULL) { return RNANDOUBLE; }
    ...
    else
    {
        ...
        ids = document->queryIntersectedEntitiesXY(qb, true, true,
                                                   referencedBlockId);
    }
    ...
    for (int col = 0; col < columnCount; col++)
    {
        for (int row = 0; row < rowCount; row++)
        {
            for (it = ids.begin(); it != ids.end(); it++)
            {
                ...
                double dist = entity->getDistanceTo(point, limited, range,
                                                    draft, strictRange);

                if (!RMath::isNormal(dist)) { continue; }

                if (dist < minDist || RMath::isNaN(minDist)) { minDist = dist; }
            }
        }
    }
...
{
    QList<RBox> *bbs;
    if (ignoreEmpty) { bbs = &boundingBoxesIgnoreEmpty; }
    else { bbs = &boundingBoxes; }

    if (!bbs->isEmpty()) { return *bbs; }

    if (document == NULL) { return QList<RBox>(); }

    QSharedPointer<RBlock> block =
            document->queryBlockDirect(referencedBlockId);
    if (block.isNull()) { return QList<RBox>(); }
        ...
        for (int row = 0; row < rowCount; row++)
        {
            for (it = ids.begin(); it != ids.end(); it++)
            {
                ...
                if (col != 0 || row != 0)
                {
                    ...
                    else { offs.x /= scaleFactors.x; }
                    if (RMath::fuzzyCompare(scaleFactors.y, 0.0))
                    {
                        offs.y = 0.0;
                    }
                    else { offs.y /= scaleFactors.y; }
                ...
                for (int i = 0; i < ebs.length(); i++)
                {
                    ...
                    RBox b(RVector::getMinimum(corners),
                           RVector::getMaximum(corners));
                    bbs->append(b);
                }
            }
        }
...
RVector RBlockReferenceData::getPointOnEntity() const
{
    ...
    QSet<REntity::Id> ids = document->queryBlockEntities(referencedBlockId);
    if (ids.isEmpty()) { return RVector::invalid; }

    RVector ret = RVector::invalid;

    QList<REntity::Id> idsOrdered =
            document->getStorage().orderBackToFront(ids);
...
RBlockReferenceData::queryEntity(REntity::Id entityId, bool transform,
                                 bool ignoreAttDef) const
{
    ...
    if (cache.contains(entityId) && !transform)
    {
        ...
        if (e->isUndone()) { return QSharedPointer<REntity>(); }
    ...
    if (!transform) { cache.insert(entityId, entity); }

    return entity;
}

RVector RBlockReferenceData::getColumnRowOffset(int col, int row,
                                                bool rotated) const
{
    if (col == 0 && row == 0) { return RVector(0, 0); }

    RVector offset;
    if (RMath::fuzzyCompare(scaleFactors.x, 0.0)) { offset.x = 0.0; }
    else
    {
        offset.x = col * columnSpacing;
        //offset.x = col*columnSpacing/scaleFactors.x;
    }
    if (RMath::fuzzyCompare(scaleFactors.y, 0.0)) { offset.y = 0.0; }
    else
    {
        offset.y = row * rowSpacing;
        //offset.y = row*rowSpacing/scaleFactors.y;
    }
    if (rotated) { offset.rotate(rotation); }
    return offset;
}

void RBlockReferenceData::applyColumnRowOffsetTo(REntity &entity, int col,
                                                 int row, bool rotated) const
{
...
bool RBlockReferenceData::applyTransformationTo(REntity &entity) const
{
    ...
    RBlockReferenceEntity *blockReference =
    ...
    if (blockReference != NULL && scaleFactors.y < 0.0)
    {
        ...
        blockReference->rotate(-2 * blockReference->getRotation(),
                               blockReference->getPosition());
...
bool RBlockReferenceData::applyTransformationTo(
        QSharedPointer<REntity> &entity) const
{
    QSharedPointer<RBlock> block =
            document->queryBlockDirect(referencedBlockId);
    if (block.isNull())
    ...
    // nested block reference with negative scale factors (flipped):
    RBlockReferenceEntity *blockReference =
            dynamic_cast<RBlockReferenceEntity *>(entity.data());
    if (blockReference != NULL && scaleFactors.y < 0.0)
    {
        blockReference->move(-block->getOrigin());
        blockReference->scale(scaleFactors);
    ...
    else
    {
        ...
        QSharedPointer<REntity> e =
                entity->scaleNonUniform(scaleFactors, RVector());
        if (!e.isNull() && e.data() != entity.data()) { entity.swap(e); }
    }
    entity->rotate(rotation);
    entity->move(position);
...
RVector RBlockReferenceData::mapToBlock(const RVector &v) const
{
    ...
    if (fabs(scaleFactors.x) > RS::PointTolerance &&
        fabs(scaleFactors.y) > RS::PointTolerance)
    {
...
QList<RRefPoint> RBlockReferenceData::getInternalReferencePoints(
        ...
        QList<REntity::Id> *subEntityIds) const
{
    ...
    QList<QSharedPointer<RShape>> shapes =
            getShapes(RDEFAULT_RBOX, false, false, &entityIds);
...
QList<RRefPoint>
RBlockReferenceData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
    Q_UNUSED(hint)
...
RBox RBlockReferenceData::getQueryBoxInBlockCoordinates(const RBox &box) const
{
    ...
    RVector::scaleList(corners,
                       RVector(1.0 / scaleFactors.x, 1.0 / scaleFactors.y));
    if (document != NULL)
    {
        QSharedPointer<RBlock> block =
                document->queryBlockDirect(referencedBlockId);
        if (!block.isNull()) { RVector::moveList(corners, block->getOrigin()); }
    }
...
RBlockReferenceData::getShapes(const RBox &queryBox, bool ignoreComplex,
                               bool segment,
                               QList<REntity::Id> *entityIds) const
{
    Q_UNUSED(segment)
    ...
    if (queryBoxBlockCoordinates.isValid() && !isArray)
    {
        ...
        ids = document->queryIntersectedEntitiesXY(
                queryBoxBlockCoordinates, true, true, referencedBlockId);
    }
    else { ids = document->queryBlockEntities(referencedBlockId); }
    ...
    for (int col = 0; col < columnCount; col++)
    {
        for (int row = 0; row < rowCount; row++)
        {
            for (it = ids.begin(); it != ids.end(); it++)
            {
                ...
                if (t == RS::EntityAttributeDefinition) { continue; }
                if (t == RS::EntityAttribute) { continue; }
                ...
                QList<QSharedPointer<RShape>> shapes =
                        entity->getShapes(queryBox, ignoreComplex);
                ret.append(shapes);
                if (entityIds != NULL)
                {
...
bool RBlockReferenceData::rotate(double rotation, const RVector &center)
{
    ...
    if (!isPixelUnit()) { this->rotation += rotation; }
    update();
    return true;
}
                                ...
                                const RVector &center)
{
    ...
    if (!isPixelUnit()) { this->scaleFactors.scale(scaleFactors); }
    update();
    return true;
}
...
{
    scaleFactors = sf;
    if (fabs(scaleFactors.x) < RS::PointTolerance) { scaleFactors.x = 1.0; }
    if (fabs(scaleFactors.y) < RS::PointTolerance) { scaleFactors.y = 1.0; }
    if (fabs(scaleFactors.z) < RS::PointTolerance) { scaleFactors.z = 1.0; }
    update();
}
