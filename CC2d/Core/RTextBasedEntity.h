...
public:
    ...
    virtual RTextBasedData &getTextData() { return getData(); }

    bool isBold() const { return getData().isBold(); }

    void setBold(bool on) { getData().setBold(on); }

    bool isItalic() const { return getData().isItalic(); }

    void setItalic(bool on) { getData().setItalic(on); }

    bool isBackward() const { return getData().isBackward(); }

    void setBackward(bool on) { getData().setBackward(on); }

    bool isUpsideDown() const { return getData().isUpsideDown(); }

    void setUpsideDown(bool on) { getData().setUpsideDown(on); }

    RVector getPosition() const { return getData().getPosition(); }
    ...
    RVector getAlignmentPoint() const { return getData().getAlignmentPoint(); }
    ...
    double getTextHeight() const { return getData().getTextHeight(); }

    void setTextHeight(double h) { getData().setTextHeight(h); }

    double getTextWidth() const { return getData().getTextWidth(); }

    double getWidth() const { return getData().getWidth(); }
    double getHeight() const { return getData().getHeight(); }

    RS::VAlign getVAlign() const { return getData().getVAlign(); }

    void setVAlign(RS::VAlign va) { getData().setVAlign(va); }

    RS::HAlign getHAlign() const { return getData().getHAlign(); }

    void setHAlign(RS::HAlign ha) { getData().setHAlign(ha); }

    QString getFontName() const { return getData().getFontName(); }
    ...
    QString getFontFile() const { return getData().getFontFile(); }
    ...
    void setAngle(double angle) { getData().setAngle(angle); }

    double getAngle() const { return getData().getAngle(); }

    void setXScale(double xScale) { getData().setXScale(xScale); }

    double getXScale() const { return getData().getXScale(); }

    void setSimple(bool on) { getData().setSimple(on); }

    bool isSimple() const { return getData().isSimple(); }
    ...
    QString getPlainText() const { return getData().getPlainText(); }

    void setText(const QString &text) { getData().setText(text); }
