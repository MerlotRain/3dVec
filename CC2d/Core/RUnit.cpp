 ...
 */
QString RUnit::getLabel(double v, RDocument &document, int precisionOverride,
                        bool forceSuppressTrailingZeroes,
                        bool onlyPreciseResult, char decimalSeparator)
{
    ...
    return RUnit::formatLinear(
            v, document.getUnit(), document.getLinearFormat(),
            precisionOverride != -1 ? precisionOverride
                                    : document.getLinearPrecision(),
            false, document.showLeadingZeroes(),
...
{
    if (unit == RS::Inch || unit == RS::Foot || unit == RS::Mile ||
        unit == RS::Mil || unit == RS::Yard)
    {
...
RS::Unit RUnit::parseUnit(const QString &str)
{
    ...
    // most common cases:
    if (l == "in" || l == "inches") { return RS::Inch; }
    if (l == "ft" || l == "feet") { return RS::Foot; }
    if (l == "mm" || l == "millimeter") { return RS::Millimeter; }
...
QString RUnit::formatLinear(double length, RS::Unit unit,
                            RS::LinearFormat format, int prec, bool showUnit,
                            ...
                            bool onlyPreciseResult, char decimalSeparator)
{
    ...
    switch (format)
    {
        ...
        case RS::Decimal:
            ret = formatDecimal(length, unit, prec, showUnit, showLeadingZeroes,
                                showTrailingZeroes, onlyPreciseResult,
                                decimalSeparator);
            break;
 ...
 * \param prec Precision of the value (e.g. 0.001 or 1/128 = 0.0078125)
 & \param showUnit Append unit to the value.
 */
QString RUnit::formatScientific(double length, RS::Unit unit, int prec,
                                bool showUnit, bool /*showLeadingZeroes*/,
                                ...
                                bool /*onlyPreciseResult*/)
{
    ...
    QString unitString = "";
    if (showUnit) { unitString = unitToSymbol(unit); }

    char format[128];
    sprintf(format, "%%.%dE%%s", prec);
#if QT_VERSION >= 0x060000
    ret = QString::asprintf(format, length,
                            (const char *) unitString.toLatin1());
#else
 ...
 * \param prec Precision of the value (e.g. 0.001)
 & \param showUnit Append unit to the value.
 */
QString RUnit::formatDecimal(double length, RS::Unit unit, int prec,
                             bool showUnit, bool showLeadingZeroes,
                             bool showTrailingZeroes,
                             bool /*onlyPreciseResult*/, char decimalSeparator)
{
    ...
    ret = doubleToString(length, prec, showLeadingZeroes, showTrailingZeroes,
                         decimalSeparator);

    if (showUnit) { ret += unitToSymbol(unit); }
 ...
 * \param prec Precision of the value (e.g. 0.001 or 1/128 = 0.0078125)
 & \param showUnit Append unit to the value.
 */
QString RUnit::formatEngineering(double length, RS::Unit unit, int prec,
                                 bool showUnit, bool showLeadingZeroes,
                                 bool showTrailingZeroes,
                                 bool onlyPreciseResult)
{
 ...
 * \param prec Precision of the value (e.g. 0.001 or 1/128 = 0.0078125)
 & \param showUnit Append unit to the value.
 */
QString RUnit::formatArchitectural(double length, RS::Unit unit, int prec,
                                   bool showUnit, bool showLeadingZeroes,
                                   bool showTrailingZeroes,
                                   bool onlyPreciseResult)
{
 ...
 * \param prec Precision of the value (e.g. 0.001 or 1/128 = 0.0078125)
 & \param showUnit Append unit to the value.
 */
QString RUnit::formatFractional(double length, RS::Unit /*unit*/, int prec,
                                bool /*showUnit*/, bool /*showLeadingZeroes*/,
                                bool /*showTrailingZeroes*/,
                                bool onlyPreciseResult)
{
    ...
    if (onlyPreciseResult)
    {
        ...
        if (!RMath::fuzzyCompare(res, length, 0.001)) { return ""; }
    }
    ...
    {
        ret = QString::asprintf("%s%d %d/%d", (const char *) neg.toLatin1(),
                                num, nominator, denominator);
    }
    else if (nominator != 0)
    {
        ret = QString::asprintf("%s%d/%d", (const char *) neg.toLatin1(),
                                nominator, denominator);
    }
    else if (num != 0)
    {
        ret = QString::asprintf("%s%d", (const char *) neg.toLatin1(), num);
    }
    else { ret = QString::asprintf("0"); }
#else
    if (num != 0 && nominator != 0)
    {
        ret.sprintf("%s%d %d/%d", (const char *) neg.toLatin1(), num, nominator,
                    denominator);
    }
    else if (nominator != 0)
    {
        ret.sprintf("%s%d/%d", (const char *) neg.toLatin1(), nominator,
                    denominator);
    }
    else if (num != 0)
    {
        ret.sprintf("%s%d", (const char *) neg.toLatin1(), num);
    }
    else { ret.sprintf("0"); }
#endif
 ...
 */
QString RUnit::formatAngle(double angle, RS::AngleFormat format, int prec,
                           bool showLeadingZeroes, bool showTrailingZeroes,
                           char decimalSeparator)
{
    ...
    switch (format)
    {
        ...
        case RS::Radians:
        case RS::Gradians:
            ret = doubleToString(value, prec, showLeadingZeroes,
                                 showTrailingZeroes, decimalSeparator);
            if (format == RS::DegreesDecimal) ret += QChar(0xB0);
            if (format == RS::Radians) ret += "r";
            if (format == RS::Gradians) ret += "g";
            break;
        ...
        case RS::DegreesMinutesSeconds:
            {
                ...
                seconds = doubleToString(vSeconds, (prec > 1 ? prec - 2 : 0),
                                         showLeadingZeroes, showTrailingZeroes);
                ...
                if (prec == 0 && vMinutes >= 30.0) { vDegrees++; }
                else if (prec == 1 && vSeconds >= 30.0) { vMinutes++; }
 ...
 */
QString RUnit::doubleToString(double value, double prec, bool showLeadingZeroes,
                              bool showTrailingZeroes, char decimalSeparator)
{
    ...
    else
    {
        ...
        ret = doubleToString(num * prec, digits, showLeadingZeroes,
                             showTrailingZeroes, decimalSeparator);
    }
                              ...
                              bool showTrailingZeroes, char decimalSeparator)
{
    ...
    if (showTrailingZeroes) { formatString = QString("%.0%1f").arg(prec); }
    else { formatString = QString("%.%1f").arg(prec); }
    ...
    if (decimalSeparator != '.') { ret.replace('.', decimalSeparator); }
