...
public:
    ...
    static RS::EntityType getRtti() { return RS::EntityUnknown; }
    ...
    virtual RS::EntityType getType() const { return getData().getType(); }

    virtual bool isPointType() const { return getData().isPointType(); }
    ...
    virtual bool isValid() const { return getData().isValid(); }
    ...
    void setUpdatesEnabled(bool on) { getData().setUpdatesEnabled(on); }

    virtual bool cloneOnChange() const { return getData().cloneOnChange(); }

    /**
     * \copydoc REntityData::setDrawOrder
     */
    void setDrawOrder(int drawOrder) { getData().setDrawOrder(drawOrder); }

    /**
     * \copydoc REntityData::getDrawOrder
     */
    int getDrawOrder() const { return getData().getDrawOrder(); }

    /**
     * \copydoc REntityData::setLayerId
     */
    void setLayerId(RLayer::Id layerId) { getData().setLayerId(layerId); }

    /**
     * \copydoc REntityData::setLayerName
     */
    void setLayerName(const QString &n) { getData().setLayerName(n); }

    /**
     * \copydoc REntityData::getLayerId
     */
    RLayer::Id getLayerId() const { return getData().getLayerId(); }
     ...
     */
    RLinetype::Id getLinetypeId() const { return getData().getLinetypeId(); }
     ...
     */
    double getLinetypeScale() const { return getData().getLinetypeScale(); }
    ...
    getLineweight(bool resolve, const QStack<REntity *> &blockRefStack) const;
     ...
     */
    void setColor(const RColor &color) { getData().setColor(color); }

    /**
     * \copydoc REntityData::getColor
     */
    RColor getColor() const { return getData().getColor(); }

    RColor getColor(const RColor &unresolvedColor,
     ...
     */
    virtual void to2D() { getData().to2D(); }

    /**
     * \copydoc REntityData::setZ
     */
    virtual void setZ(double z) { getData().setZ(z); }
     ...
     */
    RObject::Id getClosestSubEntityId(const RVector &pos,
                                      double range = RNANDOUBLE,
                                      bool ignoreComplex = false) const
    {
    ...
    getClosestShape(const RVector &pos, double range = RNANDOUBLE,
     ...
     */
    virtual QSharedPointer<RShape>
    getClosestSimpleShape(const RVector &pos, double range = RNANDOUBLE) const
    {
     ...
     */
    virtual bool isOnEntity(const RVector &point, bool limited = true,
     ...
     */
    virtual RVector
    getClosestPointOnEntity(const RVector &point, double range = RNANDOUBLE,
                            bool limited = true,
                            REntity::Id *subEntityId = NULL) const
    {
    ...
    virtual QList<RVector> getIntersectionPoints(
            const REntity &other, bool limited = true,
            const RBox &queryBox = RDEFAULT_RBOX, bool ignoreComplex = true,
            QList<QPair<REntity::Id, REntity::Id>> *entityIds = NULL) const;
     ...
     */
    virtual QList<RVector>
    getIntersectionPointsWithShape(const RShape &shape, bool limited = true,
     ...
     */
    virtual bool move(const RVector &offset) { return getData().move(offset); }
    ...
    scaleNonUniform(const RVector &scaleFactors,
                    const RVector &center = RDEFAULT_RVECTOR);
     ...
     */
    virtual bool mirror(const RLine &axis) { return getData().mirror(axis); }
     ...
     */
    virtual bool flipHorizontal() { return getData().flipHorizontal(); }

    /**
     * \copydoc REntityData::flipVertical
     */
    virtual bool flipVertical() { return getData().flipVertical(); }
     ...
     */
    void update() const { getData().update(); }

    /**
     * \copydoc REntityData::castToShape
     */
    virtual RShape *castToShape() { return getData().castToShape(); }
    ...
    virtual int getComplexity() const { return 1; }
