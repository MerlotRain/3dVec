 ...
 */
RPolyline::RPolyline() : closed(false) {}
...
{
    for (int i = 0; i < vertices.size(); i++) { vertices[i].z = z; }
}
...
bool RPolyline::isFlat() const
{
    ...
    for (int i = 0; i < vertices.size(); i++)
    {
        ...
        if (!RMath::fuzzyCompare(z, vertices[i].z)) { return false; }
    }
...
bool RPolyline::appendShape(const RShape &shape, bool prepend)
{
    ...
    if (shape.getShapeType() == RShape::Ellipse)
    {
        const REllipse *elp = dynamic_cast<const REllipse *>(&shape);
        if (elp != NULL)
        {
            RPolyline pl = elp->approximateWithArcs(100);
        ...
        if (pl != NULL)
        {
            ...
            else
            {
                for (int i = 0; i < pl->countSegments(); ++i)
                {
                    QSharedPointer<RShape> s = pl->getSegmentAt(i);
                    if (s.isNull()) { continue; }
                    setStartWidthAt(vertices.length() - 1,
    ...
    if (countVertices() > 0)
    {
        ...
        if (shape.getShapeType() == RShape::Line)
        {
            ...
            if (ips.length() == 1)
            {
                RVector ip = ips[0];
                moveEndPoint(ip);
                QSharedPointer<RShape> trimmed =
                        QSharedPointer<RShape>(shape.clone());
...
bool RPolyline::closeTrim()
{
    ...
    if (countSegments() > 1)
    {
        ...
        if (firstSegment.isNull() || lastSegment.isNull()) { return false; }

        if (firstSegment->getShapeType() == RShape::Line &&
            lastSegment->getShapeType() == RShape::Line)
        {
            QList<RVector> ips =
                             ...
                             double bulgeBefore, double bulgeAfter)
{
    ...
    if (index > 0) { bulges[index - 1] = bulgeBefore; }
...
void RPolyline::insertVertexAt(const RVector &point)
{
    ...
    if (arc1.isNull()) { setBulgeAt(index, 0.0); }
    else { setBulgeAt(index, arc1->getBulge()); }

    if (arc2.isNull()) { setBulgeAt(index + 1, 0.0); }
...
QList<RVector> RPolyline::getVertices() const { return vertices; }
...
int RPolyline::getVertexIndex(const RVector &v, double tolerance) const
{
    ...
    {
        if (vertices[i].equalsFuzzy(v, tolerance)) { return i; }
...
{
    if (vertices.size() == 0) { return RVector::invalid; }
...
int RPolyline::countVertices() const { return vertices.size(); }

void RPolyline::setBulges(const QList<double> &b) { bulges = b; }

QList<double> RPolyline::getBulges() const { return bulges; }

double RPolyline::getBulgeAt(int i) const
{
    if (i < 0 || i >= bulges.size()) { return RNANDOUBLE; }
...
bool RPolyline::hasArcSegments() const
{
    ...
    {
        if (!isStraight(bulges[i])) { return true; }
    }
...
double RPolyline::getVertexAngle(int i, RS::Orientation orientation) const
{
    ...
    QSharedPointer<RShape> prevSegment =
...
{
    for (int i = 0; i < startWidths.length(); i++) { startWidths[i] = w; }
    for (int i = 0; i < endWidths.length(); i++) { endWidths[i] = w; }
}

void RPolyline::setStartWidthAt(int i, double w)
{
    if (i < 0 || i >= startWidths.size()) { return; }
    startWidths[i] = w;
}

double RPolyline::getStartWidthAt(int i) const
{
    if (i < 0 || i >= startWidths.size()) { return -1.0; }
...
{
    if (i < 0 || i >= endWidths.size()) { return; }
    endWidths[i] = w;
}

double RPolyline::getEndWidthAt(int i) const
{
    if (i < 0 || i >= endWidths.size()) { return -1.0; }
...
bool RPolyline::hasWidths() const
{
    for (int i = 0; i < startWidths.length() && i < endWidths.length(); i++)
    {
        if (!RMath::isNaN(startWidths[i]) && startWidths[i] > 0.0)
        {
            ...
            if (i != startWidths.length() - 1 || isClosed()) { return true; }
        }
...
void RPolyline::setStartWidths(const QList<double> &sw) { startWidths = sw; }

QList<double> RPolyline::getStartWidths() const { return startWidths; }

void RPolyline::setEndWidths(const QList<double> &ew) { endWidths = ew; }

QList<double> RPolyline::getEndWidths() const { return endWidths; }
 ...
 */
void RPolyline::setClosed(bool on) { closed = on; }

/**
 * \return True if this polyline is logically marked as closed.
 */
bool RPolyline::isClosed() const { return closed; }
...
RS::Orientation RPolyline::getOrientation(bool implicitelyClosed) const
{
    ...
    if (shapeBefore.isNull() || shapeAfter.isNull()) { return RS::Any; }
...
{
    for (int i = 0; i < startWidths.length(); i++) { startWidths[i] = 0.0; }
    for (int i = 0; i < endWidths.length(); i++) { endWidths[i] = 0.0; }
}
...
{
    if (i < 0 || i > bulges.size()) { return true; }
...
QList<QSharedPointer<RShape>> RPolyline::getExploded(int segments) const
{
    ...
    if (vertices.size() <= 1) { return ret; }

    for (int i = 0; i < vertices.size(); i++)
    {
        if (!closed && i == vertices.size() - 1) { break; }
...
QList<QPair<RPolyline, RPolyline>> RPolyline::getLeftRightOutline() const
{
    ...
    else { return QList<QPair<RPolyline, RPolyline>>(); }
}
...
bool RPolyline::containsShape(const RShape &shape) const
{
    ...
    if (RShape::isPolylineShape(shape))
    {
        ...
        {
            if (contains(pl.getVertexAt(i))) { return true; }
        }
        return false;
    }
...
{
    if (polylineProxy != NULL) { return polylineProxy->getPointInside(*this); }
    return RVector::invalid;
}
...
RVector RPolyline::getEndPoint() const
{
    ...
    if (isClosed()) { return vertices.first(); }
...
void RPolyline::moveSegmentAt(int i, const RVector &offset)
{
    ...
    {
        if (closed) { moveVertexAt(0, offset); }
    }
}

double RPolyline::getDirection1() const
{
    if (vertices.size() == 0) { return RNANDOUBLE; }
...
double RPolyline::getDirection2() const
{
    ...
    if (shape.isNull()) { return RNANDOUBLE; }
...
RS::Side RPolyline::getSideOfPoint(const RVector &point) const
{
    ...
    QSharedPointer<RShape> segment = getSegmentAt(i);
    if (segment.isNull()) { return RS::NoSide; }
...
RBox RPolyline::getBoundingBox() const
{
    ...
    if (countVertices() == 1) { ret = RBox(vertices.at(0), vertices.at(0)); }
...
{
    double ret = 0.0;
    if (polylineProxy != NULL) { ret = polylineProxy->getArea(*this, 0.01); }
    return ret;
}
...
QList<double> RPolyline::getDistancesFromStart(const RVector &p) const
{
    ...
    if (ret.isEmpty()) { ret.append(getLengthTo(p, true)); }

    return ret;
}

QList<RVector> RPolyline::getEndPoints() const { return vertices; }
...
RVector RPolyline::getPointAtPercent(double p) const
{
    ...
    QList<RVector> candidates = getPointsWithDistanceToEnd(
                                                     ...
                                                     int from) const
{
    ...
    if (from & RS::AlongPolyline)
    {
        ...
        if (from & RS::FromStart)
        {
            ...
            {
                // extend at start:
                ret.append(sub.first()->getPointsWithDistanceToEnd(
            ...
            else
            {
                ...
                for (int i = 0; i < sub.length(); i++)
                {
                    ...
                    {
                        ret.append(sub[i]->getPointsWithDistanceToEnd(
        ...
        if (from & RS::FromEnd)
        {
            ...
            {
                // extend at end:
                ret.append(sub.last()->getPointsWithDistanceToEnd(distance,
                                                                  RS::FromEnd));
            }
            else
            {
...
{
    for (int i = 0; i < vertices.size(); i++) { vertices[i].move(offset); }
    return true;
}
...
bool RPolyline::scale(const RVector &scaleFactors, const RVector &center)
{
        ...
        !RMath::fuzzyCompare(scaleFactors.x, scaleFactors.y))
    {
        ...
        for (int i = 0; i < countSegments(); i++)
        {
            ...
            else { newSeg = RShape::scaleArc(*seg, scaleFactors, center); }

            if (!newSeg.isNull()) { pl.appendShape(*newSeg); }
        }
    ...
    {
        if (startWidths[i] > 0.0) { startWidths[i] *= fabs(scaleFactors.x); }
    }
    for (int i = 0; i < endWidths.size(); i++)
    {
        if (endWidths[i] > 0.0) { endWidths[i] *= fabs(scaleFactors.x); }
    }
    // factor in x or in y is negative -> mirror:
    if ((scaleFactors.x < 0) != (scaleFactors.y < 0))
    {
        for (int i = 0; i < bulges.size(); i++) { bulges[i] *= -1; }
    }
    return true;
}
...
{
    int i;
    for (i = 0; i < vertices.size(); i++) { vertices[i].mirror(axis); }
    for (i = 0; i < bulges.size(); i++) { bulges[i] *= -1; }
    return true;
}
...
bool RPolyline::reverse()
{
    ...
    if (closed) { nPolyline.convertToClosed(); }
...
RPolyline::getTransformed(const QTransform &transform) const
{
 ...
 */
QList<RVector> RPolyline::verifyTangency(double toleranceMin,
                                         double toleranceMax)
{
    if (RPolyline::hasProxy())
    {
        return RPolyline::getPolylineProxy()->verifyTangency(
                *this, toleranceMin, toleranceMax);
    }
    else { return QList<RVector>(); }
}
...
bool RPolyline::isConcave() const { return !getConcaveVertices().isEmpty(); }

QList<RVector> RPolyline::getConvexVertices(bool convex) const
{
    if (!isGeometricallyClosed()) { return QList<RVector>(); }
...
{
    if (polylineProxy != NULL) { return polylineProxy->getBaseAngle(*this); }
    return 0.0;
}

double RPolyline::getWidth() const
{
    if (polylineProxy != NULL) { return polylineProxy->getWidth(*this); }
    return 0.0;
}

bool RPolyline::setWidth(double v)
{
    if (polylineProxy != NULL) { return polylineProxy->setWidth(*this, v); }
    return false;
}

double RPolyline::getHeight() const
{
    if (polylineProxy != NULL) { return polylineProxy->getHeight(*this); }
    return 0.0;
}

bool RPolyline::setHeight(double v)
{
    if (polylineProxy != NULL) { return polylineProxy->setHeight(*this, v); }
    return false;
}

QList<RPolyline> RPolyline::morph(const RPolyline &target, int steps,
                                  RS::Easing easing, bool zLinear,
