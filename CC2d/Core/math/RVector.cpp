...
const RVector RVector::nanVector =
        RVector(RNANDOUBLE, RNANDOUBLE, RNANDOUBLE, true);
...
{
    if (tuples.length() > 0) { x = tuples[0]; }
    if (tuples.length() > 1) { y = tuples[1]; }
    if (tuples.length() > 2) { z = tuples[2]; }
    valid = true;
}
...
double RVector::getX() { return x; }

void RVector::setY(double y) { this->y = y; }

double RVector::getY() { return y; }

void RVector::setZ(double z) { this->z = z; }

double RVector::getZ() { return z; }
...
double RVector::getAngleTo(const RVector &v) const
{
    ...
    else { return (v.get2D() - get2D()).getAngle(); }
}
...
{
    return (qAbs(x - v.x) < tol && qAbs(y - v.y) < tol && qAbs(z - v.z) < tol &&
            valid == v.valid);
}
...
{
    return (qAbs(x - v.x) < tol && qAbs(y - v.y) < tol && valid == v.valid);
}
...
{
    if (!valid || !v.valid) { return RNANDOUBLE; }
    else { return (*this - v).getMagnitude2D(); }
}
...
{
    for (int i = 0; i < list.length(); i++) { list[i].move(offset); }
}
...
RVector RVector::rotate3D(const RLine &axis, double rotation)
{
    ...
    QQuaternion quat = QQuaternion::fromAxisAndAngle(ax.x, ax.y, ax.z,
                                                     RMath::rad2deg(rotation));
...
{
    for (int i = 0; i < list.length(); i++) { list[i].rotate(rotation); }
}
...
void RVector::scaleList(QList<RVector> &list, double factor,
                        const RVector &center)
{
    for (int i = 0; i < list.length(); i++) { list[i].scale(factor, center); }
}

void RVector::scaleList(QList<RVector> &list, const RVector &factors,
                        const RVector &center)
{
    for (int i = 0; i < list.length(); i++) { list[i].scale(factors, center); }
}
...
RVector RVector::isoProject(RS::IsoProjectionType type)
{
    ...
    else { return axonometricProjection(type); }
}
...
RVector RVector::axonometricProjection(RS::IsoProjectionType type)
{
    ...
    RMatrix iso = RMatrix::create3x3(sx * cos(a), -sy * cos(b), 0, sx * sin(a),
                                     sy * sin(b), sz, 0, 0, 0);
    RMatrix input;
    switch (projection)
    {
...
RVector RVector::obliqueProjection(RS::IsoProjectionType type)
{
    ...
    if (method == RS::Cabinet || method == RS::Cabinet30) { f = 0.5; }

    int projection = type & 0x0000ffff;
    double a;
    if (method == RS::Cabinet30) { a = RMath::deg2rad(30); }
    else { a = RMath::deg2rad(45); }
    RVector p;
 ...
 */
RVector RVector::getNegated() const { return RVector(-x, -y, -z, valid); }
...
bool RVector::containsFuzzy(const QList<RVector> &vectors, const RVector &v,
                            double tol)
{
    return findFirstFuzzy(vectors, v, tol) != -1;
}

int RVector::findFirstFuzzy(const QList<RVector> &vectors, const RVector &v,
                            double tol)
{
...
{
    return RVector(qMin(v1.x, v2.x), qMin(v1.y, v2.y), qMin(v1.z, v2.z),
                   v1.valid && v2.valid);
}
...
{
    return RVector(qMax(v1.x, v2.x), qMax(v1.y, v2.y), qMax(v1.z, v2.z),
                   v1.valid && v2.valid);
}
...
RVector RVector::getAverage(const QList<RVector> &vectors)
{
    ...
    for (int i = 0; i < vectors.length(); i++) { sum += vectors[i]; }
...
QList<RVector> RVector::getUnion(const QList<RVector> &vectorsA,
                                 const QList<RVector> &vectorsB, double tol)
{
    QList<RVector> ret;
...
{
    return RVector(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
...
int RVector::getClosestIndex(const QList<RVector> &list, bool ignoreZ) const
{
    ...
    for (int i = 0; i < list.size(); ++i)
    {
        ...
        {
            double dist;
            if (ignoreZ) { dist = getDistanceTo2D(list[i]); }
            else { dist = getDistanceTo(list[i]); }
            if (dist < minDist)
            {
                minDist = dist;
                index = i;
...
QList<RVector> RVector::getSortedLeftRightTopBottom(const QList<RVector> &list)
{
    ...
    std::sort(ret.begin(), ret.end(),
              RVector::RVectorLeftRightTopBottomSort::lessThan);
    return ret;
}
...
QList<RVector> RVector::getSortedByAngle(const QList<RVector> &list,
                                         const RVector &center, double angle)
{
...
bool RVector::RVectorDistanceSort::lessThan(const RVector &v1,
                                            const RVector &v2)
{
    return v.getDistanceTo(v1) < v.getDistanceTo(v2);
}

bool RVector::RVectorLeftRightTopBottomSort::lessThan(const RVector &v1,
                                                      const RVector &v2)
{
