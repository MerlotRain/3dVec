...
RTriangle RTriangle::createArrow(const RVector &position, double direction,
                                 double arrowSize)
{
...
RBox RTriangle::getBoundingBox() const
{
    return RBox(RVector::getMinimum(RVector::getMinimum(corner[0], corner[1]),
                                    corner[2]),
                RVector::getMaximum(RVector::getMaximum(corner[0], corner[1]),
                                    corner[2]));
}
...
{
    if (i < 0 || i > 2) { return RVector::invalid; }
...
void RTriangle::setCorners(const RVector &c1, const RVector &c2,
                           const RVector &c3)
{
    corner[0] = c1;
    corner[1] = c2;
...
double RTriangle::getDistanceTo(const RVector &point, bool limited,
...
RVector RTriangle::getVectorTo(const RVector &point, bool limited,
                               double strictRange) const
{
    //assert(false);
    ...
    if (m1 < m2 && m1 < m3) { return v1; }
    else if (m2 < m3) { return v2; }
    else { return v3; }
}
...
QList<RVector> RTriangle::getCenterPoints() const { return getMiddlePoints(); }
...
{
    return RVector::getCrossProduct(corner[0] - corner[2],
                                    corner[1] - corner[2]);
}
...
QList<QSharedPointer<RShape>> RTriangle::getExploded(int segments) const
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ret.append(QSharedPointer<RShape>(
                new RLine(corner[i], corner[(i + 1) % 3])));
    }
