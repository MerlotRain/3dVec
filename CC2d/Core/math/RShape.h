...
public:
    ...
    virtual RShape::Type getShapeType() const { return Unknown; }
    ...
    virtual double getDistanceTo(const RVector &point, bool limited = true,
                                 double strictRange = RMAXDOUBLE) const;
    virtual double getMaxDistanceTo(const QList<RVector> &points,
                                    bool limited = true,
                                    double strictRange = RMAXDOUBLE) const;
    ...
    filterOnShape(const QList<RVector> &pointList, bool limited = true,
                  double tolerance = RDEFAULT_TOLERANCE_1E_MIN4) const;
    ...
    {
        QList<RVector> res = getPointsWithDistanceToEnd(
                distance, RS::FromStart | RS::AlongPolyline);
        if (res.isEmpty()) { return RVector::invalid; }
        return res[0];
    }
    ...
    {
        QList<RVector> res = getPointsWithDistanceToEnd(
                distance, RS::FromEnd | RS::AlongPolyline);
        if (res.isEmpty()) { return RVector::invalid; }
        return res[0];
    }
    ...
    virtual double getDirection1() const { return RNANDOUBLE; }
    virtual double getDirection2() const { return RNANDOUBLE; }
    ...
    virtual RVector getStartPoint() const { return RVector::invalid; }
    virtual RVector getEndPoint() const { return RVector::invalid; }
    virtual RVector getMiddlePoint() const { return RVector::invalid; }
    ...
    virtual bool rotate(double rotation,
                        const RVector &center = RDEFAULT_RVECTOR) = 0;
    ...
    getTransformed(const QTransform &transform) const = 0;

    static QList<QSharedPointer<RShape>>
    getOrderedShapes(const QList<QSharedPointer<RShape>> &shapes);
    ...
    static QList<QSharedPointer<RShape>>
    getReversedShapeList(const QList<QSharedPointer<RShape>> &shapes);

    virtual QList<QSharedPointer<RShape>>
    splitAt(const QList<RVector> &points) const;

    static QList<QSharedPointer<RShape>> trim(const RShape &trimShape,
    ...
    static QList<QSharedPointer<RShape>>
    roundCorners(const QList<QSharedPointer<RShape>> &shapes, double radius);

    static QList<QSharedPointer<RShape>>
    roundShapes(const QSharedPointer<RShape> shape1, const RVector &clickPos1,
                const QSharedPointer<RShape> shape2, const RVector &clickPos2,
                bool trim, bool samePolyline, double radius,
                const RVector &pos);
     ...
     */
    static QSharedPointer<RShape>
    transformArc(const RShape &shape, RShapeTransformation &transformation);
    static QSharedPointer<RShape>
