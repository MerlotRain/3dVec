 ...
 */
double RShape::getMaxDistanceTo(const QList<RVector> &points, bool limited,
                                double strictRange) const
{
    double ret = 0.0;
...
bool RShape::equals(const RShape &other, double tolerance) const
{
    ...
    {
        if (!vList1[i].equalsFuzzy(vList2[i], tolerance)) { return false; }
    }

    QList<double> dList1 = getDoubleProperties();
    QList<double> dList2 = other.getDoubleProperties();
    if (dList1.length() != dList2.length()) { return false; }
    ...
    if (bList1.length() != bList2.length()) { return false; }
    for (int i = 0; i < bList1.length(); i++)
    {
        if (bList1[i] != bList2[i]) { return false; }
    }
...
RVector RShape::getPointAtPercent(double p) const
{
    ...
    if (candidates.length() != 1) { return RVector::invalid; }
 ...
 *      other shape.
 */
QList<RVector> RShape::getIntersectionPoints(const RShape &other, bool limited,
                                             ...
                                             bool same, bool force)
{
...

    {
        ...
        if (line1 != NULL)
        {
            ...
            const RTriangle *triangle2 =
                    dynamic_cast<const RTriangle *>(&shape2);
            if (triangle2 != NULL)
            {
            ...
            if (ray2 != NULL)
            {
                QList<RVector> ret = getIntersectionPointsLL(
...

    {
        ...
        if (xline1 != NULL)
        {
            ...
            if (triangle2 != NULL)
            {
                return getIntersectionPointsLT(line1, *triangle2, false,
                                               limited);
            }
            const RRay *ray2 = dynamic_cast<const RRay *>(&shape2);
            if (ray2 != NULL)
...

    {
        ...
        if (arc1 != NULL)
        {
            ...
            if (ray2 != NULL)
            {
                QList<RVector> ret = getIntersectionPointsLA(
...

    {
        ...
        if (circle1 != NULL)
        {
            ...
            if (ray2 != NULL)
            {
                QList<RVector> ret = getIntersectionPointsLC(
...

    {
        ...
        if (ellipse1 != NULL)
        {
            ...
            if (ray2 != NULL)
            {
                QList<RVector> ret = getIntersectionPointsLE(
...

    {
        ...
        if (triangle1 != NULL)
        {
            ...
            if (ray2 != NULL)
            {
                QList<RVector> ret = getIntersectionPointsLT(
                        ray2->getLineShape(), *triangle1, false, limited);
                if (limited) ret = ray2->filterOnShape(ret, true);
                return ret;
            }
            const RXLine *xline2 = dynamic_cast<const RXLine *>(&shape2);
            if (xline2 != NULL)
            {
                return getIntersectionPointsLT(xline2->getLineShape(),
                                               *triangle1, false, limited);
            }
        }
    }
...

    {
        ...
        if (explodable1 != NULL)
        {
            if (!same)
            {
                ...
                if (ray2 != NULL)
                {
                    QList<RVector> ret = getIntersectionPointsLX(
...
QList<RVector> RShape::getIntersectionPointsLA(const RLine &line1,
                                               const RArc &arc2, bool limited1,
                                               bool limited2)
{

    QList<RVector> candidates = RShape::getIntersectionPointsLC(
    ...
    {
        if (arc2.isOnShape(candidates[i])) { res.append(candidates[i]); }
    }
                                               ...
                                               bool limited)
{
    ...
    if (RMath::fuzzyCompare(dist, circle2.getRadius(), 1.0e-6))
    {
        ...
        if (!limited || line1.isOnShape(sol, true, 1.0e-6)) { res.append(sol); }
        // ret.setTangent(true);
        return res;
    }
    ...
    double term = RMath::pow(RVector::getDotProduct(d, delta), 2.0) -
                  RMath::pow(d.getMagnitude(), 2.0) *
                          (RMath::pow(delta.getMagnitude(), 2.0) -
                           RMath::pow(circle2.getRadius(), 2.0));
    ...
    if (!limited || line1.isOnShape(sol1, true, 1.0e-6)) { res.append(sol1); }
    if (sol2.isValid())
    {
    ...
    if (res.length() == 2 && res[0].equalsFuzzy(res[1])) { res.removeLast(); }
                                               ...
                                               bool limited1, bool limited2)
{
    ...
    if (normal.getMagnitude() < 1.0e-12) { return res; }

    if (line1.getLength() < 1.0e-12) { return res; }

    double t = RVector::getDotProduct(normal, triangle2.getCorner(2) -
                                                      line1.getStartPoint()) /
               RVector::getDotProduct(
                       normal, (line1.getEndPoint() - line1.getStartPoint()));
    ...
    // intersection point:
    RVector ip = line1.getStartPoint() +
                 (line1.getEndPoint() - line1.getStartPoint()) * t;

    // check if intersection point is inside the triangle:
    if (!limited2 || triangle2.isPointInTriangle(ip)) { res.push_back(ip); }
                                               ...
                                               bool limited)
{
    ...
    for (it = sub.begin(); it != sub.end(); ++it)
    {
        ...
        if (!pLine2.isNull())
        {
            ...
            res.append(RShape::getIntersectionPointsLL(line1, line2, limited,
                                                       true));
            continue;
        }
        ...
        if (!pArc2.isNull())
        {
            ...
            res.append(RShape::getIntersectionPointsLA(line1, arc2, limited,
                                                       true));
            continue;
        }
    }
                                               ...
                                               const RArc &arc2, bool limited)
{
    ...
    QList<RVector> candidates = RShape::getIntersectionPoints(
                                               ...
                                               bool limited)
{
    ...
    for (int i = 0; i < candidates.count(); i++)
    {
        if (arc1.isOnShape(candidates[i])) { res.append(candidates[i]); }
    }
    // ret.setTangent(tangent);
...
QList<RVector> RShape::getIntersectionPointsAE(const RArc &arc1,
                                               ...
                                               bool limited)
{
    QList<RVector> candidates = RShape::getIntersectionPointsCE(
                                               ...
                                               bool limited)
{
    ...
    QList<RVector> r = plane.getIntersectionPoints(
            RLine(triangle2.getCorner(0), triangle2.getCorner(1)));
    r.append(plane.getIntersectionPoints(
            RLine(triangle2.getCorner(1), triangle2.getCorner(2))));
    r.append(plane.getIntersectionPoints(
            RLine(triangle2.getCorner(2), triangle2.getCorner(0))));
    if (r.size() < 2) { return QList<RVector>(); }
                                               ...
                                               const RCircle &circle2)
{
    ...
    if (sol1.equalsFuzzy(sol2, tol)) { res.append(sol1); }
    else
    {
        res.append(sol1);
        res.append(sol2);
                                               ...
                                               const REllipse &ellipse2)
{
    ...
    if (fabs(phi_1) > twopi) { phi_1 = fmod(phi_1, twopi); }
    if (fabs(phi_2) > twopi) { phi_2 = fmod(phi_2, twopi); }
    ...
    PHI_2R = phi_2 - phi_1;
    if (fabs(PHI_2R) > twopi) { PHI_2R = fmod(PHI_2R, twopi); }
    ...
    for (it1 = sub1.begin(), c1 = 0; it1 != sub1.end(); ++it1, ++c1)
    {
        for (it2 = sub2.begin(), c2 = 0; it2 != sub2.end(); ++it2, ++c2)
        {
            ...
            if (same)
            {
                ...
                if (qAbs(c1 - c2) <= 1) { continue; }
            }

            res.append(
                    RShape::getIntersectionPoints(*it1->data(), *it2->data()));
        }
    }
...
RShape::getOrderedShapes(const QList<QSharedPointer<RShape>> &shapes)
{
    ...
    for (int i = 0; i < shapes.size(); ++i)
    {
        ...
        if (shape->isDirected())
        {
            ...
            do {
                ...
                {
                    if (traversed.contains(k)) { continue; }
    ...
    for (int i = 0; i < boundary.size(); ++i)
    {
        ...
        do {
            ...
            if (!circle.isNull())
            {
                ...
                {
                    qWarning()
                            << "RShape::order: loop not closed: circle in loop";
                    return false;
                }
                loop.removeAt(k);
            ...
            if (!ellipseArc.isNull() && ellipseArc->isFullEllipse())
            {
                if (k != 0 || loop.size() != 1)
                {
                    qWarning() << "RShape::order: loop not closed: full "
                                  "ellipse in loop";
                    return false;
                }
                loop.removeAt(k);
...
double RShape::ellipse2tr(double x, double y, double AA, double BB, double CC,
                                                     ...
                                                     const RVector &position)
{
    ...
    if (!shape.isDirected()) { return ret; }

    QList<RS::Side> sides;
    if (position.isValid()) { sides.append(shape.getSideOfPoint(position)); }
    else
    {
    ...
    for (int i = 0; i < sides.length(); i++)
    {
        ...
        double a;
        if (side == RS::LeftHand) { a = shape.getDirection1() + M_PI / 2.0; }
        else { a = shape.getDirection1() - M_PI / 2.0; }
                                                    ...
                                                    const RVector &position)
{
    ...
    else
    {
        ...
        else
        {
            if (!reversed)
            {
                if (side == RS::LeftHand) { insides.append(true); }
                else { insides.append(false); }
            }
            else
            {
                if (side == RS::RightHand) { insides.append(true); }
                else { insides.append(false); }
            }
        }
    }
...
QList<QSharedPointer<RShape>>
RShape::getReversedShapeList(const QList<QSharedPointer<RShape>> &shapes)
{
    QList<QSharedPointer<RShape>> ret;
    ...
    for (int i = shapes.length() - 1; i >= 0; i--)
    {
        QSharedPointer<RShape> shape =
                QSharedPointer<RShape>(shapes[i]->clone());
        shape->reverse();
        ret.append(shape);
    }
 ...
 */
QList<QSharedPointer<RShape>>
RShape::splitAt(const QList<RVector> &points) const
{
    Q_UNUSED(points)
                                           ...
                                           bool trimBoth, bool samePolyline)
{
    ...
    QSharedPointer<RShape> trimShapeSimple =
            QSharedPointer<RShape>(trimShape.clone());
    if (isPolylineShape(trimShape))
    {
        ...
        if (i1 < 0) { return QList<QSharedPointer<RShape>>(); }
        segment = polyline.getSegmentAt(i1);
        if (segment.isNull()) { return QList<QSharedPointer<RShape>>(); }
        trimShapeSimple = segment;
    }

    int i2 = 0;
    QSharedPointer<RShape> limitingShapeSimple =
            QSharedPointer<RShape>(limitingShape.clone());
    if (isPolylineShape(limitingShape))
    {
        ...
        if (i2 < 0) { return QList<QSharedPointer<RShape>>(); }
    ...
    if (sol.isEmpty()) { return QList<QSharedPointer<RShape>>(); }
    ...
    if (RShape::isCircleShape(trimShape))
    {
        ...
        trimmedTrimShape =
                QSharedPointer<RShape>(new RArc(c, r, a1, a2, false));
    }
    else if (RShape::isFullEllipseShape(trimShape))
    {
        ...
        a2 = RMath::getNormalizedAngle(am + M_PI / 2);
        trimmedTrimShape =
                QSharedPointer<RShape>(new REllipse(c, mp, r, a1, a2, false));
    }
    else
    {
        if (samePolyline)
        ...
        else { trimmedTrimShape = QSharedPointer<RShape>(trimShape.clone()); }
    }
    ...
    if (trimBoth)
    {
        ...
        else
        {
            ...
            {
                trimmedLimitingShape =
                        QSharedPointer<RShape>(limitingShapeSimple->clone());
            }
            else
            {
                trimmedLimitingShape =
    ...
    RVector is2;
    if (sol.length() == 1 || isIdx != 0) { is2 = sol[0]; }
    else { is2 = sol[1]; }
    ...
    {
        QSharedPointer<const RXLine> xline =
                trimmedTrimShape.dynamicCast<const RXLine>();
        trimmedTrimShape = QSharedPointer<RShape>(
    ...
    if (trimBoth && !trimmedLimitingShape.isNull())
    {
        ...
        {
            QSharedPointer<RXLine> xline =
                    trimmedLimitingShape.dynamicCast<RXLine>();
            trimmedLimitingShape = QSharedPointer<RShape>(new RRay(
        ...
        else if (isRayShape(*trimmedLimitingShape) && ending2 == RS::EndingEnd)
        {
            ...
            trimmedLimitingShape = QSharedPointer<RShape>(
                    new RLine(ray->getBasePoint(), ray->getSecondPoint()));
        }
    }
        ...
        bool trim, bool samePolyline, double radius, const RVector &pos)
{
    ...
    if (shape1.isNull() || !clickPos1.isValid() || shape2.isNull() ||
        !clickPos2.isValid())
    {
        return ret;
    }
    ...
    QList<QSharedPointer<RShape>> parallels1 =
            simpleShape1->getOffsetShapes(radius, 1, RS::NoSide, p);
    QList<QSharedPointer<RShape>> parallels2 =
            simpleShape2->getOffsetShapes(radius, 1, RS::NoSide, p);

    if (parallels1.length() != 1 || parallels2.length() != 1) { return ret; }

    QSharedPointer<RShape> parallel1 = parallels1[0];
    QSharedPointer<RShape> parallel2 = parallels2[0];

    QList<RVector> ipParallel =
            parallel1->getIntersectionPoints(*parallel2.data(), false);
    if (ipParallel.isEmpty()) { return ret; }

    QList<RVector> sol2 =
            simpleShape1->getIntersectionPoints(*simpleShape2.data(), false);
    ...
    if (samePolyline)
    {
        QSharedPointer<RPolyline> pl1 = shape1.dynamicCast<RPolyline>();
        RPolyline pl =
                pl1->modifyPolylineCorner(*trimmed1.data(), ending1, i1,
                                          *trimmed2.data(), ending2, i2, &arc);
        return QList<QSharedPointer<RShape>>()
               << QSharedPointer<RShape>(new RPolyline(pl));
    }
    ...
    if (RShape::isArcShape(*trimmed1))
    {
        ...
        if (RMath::fuzzyAngleCompare(arc1->getStartAngle(),
                                     arc1->getEndAngle()))
        {
    ...
    if (RShape::isArcShape(*trimmed2))
    {
        ...
        if (RMath::fuzzyAngleCompare(arc2->getStartAngle(),
                                     arc2->getEndAngle()))
        {
            // trimmed shape is full circle (two shapes are tangential):
            return ret;
...
RShape::transformArc(const RShape &shape, RShapeTransformation &transformation)
{
    RVector r1, r2;
    RVector c;
    ...
    if (isArcShape(shape) || isEllipseShape(shape))
    {
        ...
        if (d1 < d2) { ellipse.setReversed(false); }
    }
...
QSharedPointer<RShape>
RShape::ellipseToArcCircleEllipse(const REllipse &ellipse)
{
    if (ellipse.isCircular())
    {
        ...
        {
            return QSharedPointer<RShape>(
                    new RCircle(ellipse.getCenter(), ellipse.getMajorRadius()));
        }
        else
        {
            RVector c = ellipse.getCenter();
            QSharedPointer<RArc> ret(
                    new RArc(c, ellipse.getMajorRadius(),
    ...
    else { return QSharedPointer<RShape>(ellipse.clone()); }
}
