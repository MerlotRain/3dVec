 ...
 */
RColor::RColor() : QColor(), mode(RColor::Fixed) {}
...
RColor::RColor(RColor::Mode mode) : QColor(), mode(mode) {}
...
RColor RColor::getHighlighted(const RColor &color, const QColor &bgColor,
                              int minDist)
{
    if (!color.isValid()) { return Qt::gray; }
    ...
    if (d3 >= minDist) { ret.setHsv(color.hue(), color.saturation(), 255); }
    ...
    {
        ret.setHsv(color.hue(), color.saturation(),
                   qMin(vColor + minDist, 255));
    }
    else
    {
...
RColor RColor::getFaded(const RColor &color, const QColor &bgColor,
                        double factor)
{
    RColor ret = color;
...
QString RColor::getName() const
{
    ...
    while (it.hasNext())
    {
        ...
        if (p.second == *this) { return p.first; }
    }
...
QStringList RColor::getNameList(bool onlyFixed)
{
    init();
    QStringList l;
    for (int i = 0; i < list.length(); i++) { l.append(list[i].first); }
    if (!onlyFixed) { return l; }
...
QList<RColor> RColor::getColorList(bool onlyFixed)
{
    init();
    QList<RColor> l;
    for (int i = 0; i < list.length(); i++) { l.append(list[i].second); }
    if (!onlyFixed) { return l; }
...
{
    return RColor((code & 0x0000ff) >> 0, (code & 0x00ff00) >> 8,
                  (code & 0xff0000) >> 16);
}
...
QIcon RColor::getIcon(const RColor &color, const QSize &size)
{
    ...
    if (iconMap.contains(QPair<RColor, QPair<int, int>>(
                color, QPair<int, int>(size.width(), size.height()))))
    {
        return iconMap[QPair<RColor, QPair<int, int>>(
                color, QPair<int, int>(size.width(), size.height()))];
    }
    ...
    iconMap.insert(QPair<RColor, QPair<int, int>>(
                           color, QPair<int, int>(size.width(), size.height())),
                   ret);
    return ret;
}
