...
RObject::RObject(RDocument *document)
    : document(document), objectId(INVALID_ID), handle(INVALID_HANDLE),
      flags(NoFlags)
{
...
void RObject::setDocument(RDocument *document) { this->document = document; }
...
void RObject::init()
{
    ...
    RObject::PropertyHandle.generateId(RObject::getRtti(), "",
                                       QT_TRANSLATE_NOOP("REntity", "Handle"));
    RObject::PropertyProtected.generateId(
            RObject::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Protected"));
    RObject::PropertySelected.generateId(
            RObject::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Selected"));
    RObject::PropertyInvisible.generateId(
            RObject::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Object Invisible"));
    RObject::PropertyWorkingSet.generateId(
            RObject::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Working Set"));
}
        ...
        if (customProperties.contains(appId))
        {
            ...
            if (vm.contains(name))
            {
                if (RS::getMetaType(vm.value(name)) == RS::Int)
                {
                    return qMakePair(
                            vm.value(name),
                            RPropertyAttributes(RPropertyAttributes::Custom |
                                                RPropertyAttributes::Integer));
                }
                else
                {
                    return qMakePair(
                          ...
                          RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMemberFlag(RObject::Protect, value,
                                        PropertyProtected == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RObject::Selected, value,
                                        PropertySelected == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RObject::Invisible, value,
                                        PropertyInvisible == propertyTypeId);
    ret = ret || RObject::setMemberFlag(RObject::WorkingSet, value,
                                        PropertyWorkingSet == propertyTypeId);
    ...
    if (propertyTypeId.getId() == RPropertyTypeId::INVALID_ID)
    {
        ...
        if (value.isValid())
        {
            setCustomProperty(propertyTypeId.getCustomPropertyTitle(),
                              propertyTypeId.getCustomPropertyName(), value);
        }
        else
        {
            removeCustomProperty(propertyTypeId.getCustomPropertyTitle(),
                                 propertyTypeId.getCustomPropertyName());
        }
        return true;
    }
...
bool RObject::setMember(double &variable, const QVariant &value, bool condition)
{
    ...
    if (!ok)
    {
        qWarning() << QString("RObject::setMember: '%1' is not a double")
                              .arg(value.toString());
        return false;
    }
    variable = d;
...
bool RObject::setMember(int &variable, const QVariant &value, bool condition)
{
    ...
    if (!ok)
    {
        qWarning() << QString("RObject::setMember: '%1' is not an integer")
                              .arg(value.toString());
        return false;
    }
    variable = n;
...
RObject::getPropertyTypeIds(RPropertyAttributes::Option option) const
{
    QSet<RPropertyTypeId> ret =
            RPropertyTypeId::getPropertyTypeIds(getType(), option);
...
{
    if (!customProperties.contains(title)) { return false; }
 ...
 */
QVariant RObject::getCustomProperty(const QString &title, const QString &key,
                                    const QVariant &defaultValue) const
{
    if (!customProperties.contains(title)) { return defaultValue; }
...
int RObject::getCustomIntProperty(const QString &title, const QString &key,
...
bool RObject::getCustomBoolProperty(const QString &title, const QString &key,
 ...
 */
void RObject::setCustomProperty(const QString &title, const QString &key,
...
{
    if (!customProperties.contains(title)) { return; }
    customProperties[title].remove(key);
    if (customProperties[title].isEmpty()) { customProperties.remove(title); }
}
...
{
    if (!customProperties.contains(title)) { return QStringList(); }
...
void RObject::copyCustomPropertiesFrom(RObject *other, const QString &title,
                                       ...
                                       const QString &mapKeyFrom,
                                       const QString &mapKeyTo)
{
    ...
    for (it = otherProperties.begin(); it != otherProperties.end(); it++)
    {
        ...
        if (title.isNull() || otherTitle == title)
        {
            ...
            for (it2 = otherMap.begin(); it2 != otherMap.end(); it2++)
            {
                ...
                if (ignoreList.contains(otherKey)) { continue; }
                                          ...
                                          const RPropertyAttributes &att)
{
    ...
    {
        customPropertyAttributes.insert(title,
                                        QMap<QString, RPropertyAttributes>());
    }
