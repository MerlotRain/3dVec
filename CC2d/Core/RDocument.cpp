 ...
 */
RDocument::RDocument(RStorage &storage, RSpatialIndex &spatialIndex,
                     bool beforeLoad)
    : storage(storage), spatialIndex(spatialIndex),
      disableSpatialIndicesByBlock(false), transactionStack(storage),
      linetypeByLayerId(RObject::INVALID_ID),
      linetypeByBlockId(RObject::INVALID_ID), autoTransactionGroup(false)
{

    storage.setDocument(this);
    init(beforeLoad);
    //::incCounter("RDocument");
}
...
void RDocument::initLinetypes(RTransaction *transaction)
{
    ...
    for (int i = 0; i < lts.length(); i++) { transaction->addObject(lts[i]); }
...
QList<QSharedPointer<RObject>> RDocument::getDefaultLinetypes()
{
    ...
    else { patternList = RLinetypeListImperial::getNames(); }
    for (int i = 0; i < patternList.length(); i++)
    {
        ...
        else { pattern = RLinetypeListImperial::get(name); }
...
RS::Unit RDocument::getUnit() const { return storage.getUnit(); }
...
void RDocument::setMeasurement(RS::Measurement m, RTransaction *transaction)
{
    ...
    while (i.hasNext())
    {
        ...
        if (entity->isUndone()) { continue; }
        if (entity->getType() != RS::EntityHatch) { continue; }
        qDebug() << "update hatch";
        entity->update();
...
bool RDocument::isMetric() const
{
    ...
    if (m == RS::UnknownMeasurement) { return RUnit::isMetric(getUnit()); }
...
{
    return RUnit::formatLinear(value, getUnit(), getLinearFormat(),
                               getLinearPrecision(), false, showLeadingZeroes(),
                               showTrailingZeroes(), false);
}

QString RDocument::formatAngle(double value)
{
    return RUnit::formatAngle(value, getAngleFormat(), getAnglePrecision(),
 ...
 */
QStringList RDocument::getVariables() const { return storage.getVariables(); }
                                 ...
                                 RTransaction *transaction)
{
    ...
    if (key == RS::MEASUREMENT) { initLinetypes(transaction); }
}
                                     ...
                                     const QVariant &defaultValue) const
{
    ...
    if (!ret.isValid()) { ret = defaultValue; }
    return ret;
}
 ...
 */
RColor RDocument::getCurrentColor() const { return storage.getCurrentColor(); }
...
void RDocument::setCurrentBlock(const QString &blockName)
{
    ...
    if (id == RBlock::INVALID_ID) { return; }
    setCurrentBlock(id);
}
...
void RDocument::unsetCurrentViewport() { storage.unsetCurrentViewport(); }
...
QString RDocument::getTempBlockName() const
{
    do {
        ...
        int p2 = qrand() % 100000;
#endif
        QString blockName = QString("A$C%1%2")
                                    .arg(p1, 5, 10, QChar('0'))
                                    .arg(p2, 5, 10, QChar('0'));
        if (!hasBlock(blockName)) { return blockName; }
    } while (true);
}
 ...
 */
QSet<QString> RDocument::getViewNames() const { return storage.getViewNames(); }
 ...
 */
bool RDocument::hasLayerStates() const { return storage.hasLayerStates(); }
...
RLayer::Id RDocument::getLayer0Id() const { return storage.getLayer0Id(); }
...
void RDocument::setFileName(const QString &fn) { fileName = fn; }

QString RDocument::getFileName() const { return fileName; }

void RDocument::setFileVersion(const QString &fv) { fileVersion = fv; }

QString RDocument::getFileVersion() const { return fileVersion; }

void RDocument::resetTransactionStack() { transactionStack.reset(); }
 ...
 */
void RDocument::startTransactionGroup() { storage.startTransactionGroup(); }
 ...
 */
RStorage &RDocument::getStorage() { return storage; }

const RStorage &RDocument::getStorage() const { return storage; }

/**
 * \return Reference to the spatial index.
 */
RSpatialIndex &RDocument::getSpatialIndex() { return spatialIndex; }

const RSpatialIndex &RDocument::getSpatialIndex() const { return spatialIndex; }

RSpatialIndex *RDocument::getSpatialIndexForBlock(RBlock::Id blockId) const
{
    if (disableSpatialIndicesByBlock) { return &spatialIndex; }
 ...
 */
RTransactionStack &RDocument::getTransactionStack() { return transactionStack; }
 ...
 */
QSet<REntity::Id> RDocument::queryAllEntities(bool undone, bool allBlocks,
                                              RS::EntityType type) const
{
 ...
 */
QSet<REntity::Id> RDocument::querySelectedLayerEntities(RLayer::Id layerId,
                                                        bool allBlocks) const
{
 ...
 */
REntity::Id RDocument::queryClosestXY(const RVector &wcsPosition, double range,
                                      bool draft, double strictRange,
                                      bool includeLockedLayers,
                                      bool selectedOnly)
{

    RVector rangeV(range, range);

    // find entities that are within given maximum distance
    //   (approximation based on bounding boxes):
    QSet<REntity::Id> candidates = queryIntersectedEntitiesXY(
            RBox(wcsPosition - rangeV, wcsPosition + rangeV), true,
            includeLockedLayers, RBlock::INVALID_ID,
            RDEFAULT_QLIST_RS_ENTITYTYPE, selectedOnly);

    if (candidates.isEmpty()) { return REntity::INVALID_ID; }
...
QPair<REntity::Id, QSet<int>> RDocument::queryClosestXYWithIndices(
        const RVector &wcsPosition, double range, bool draft,
        double strictRange, bool includeLockedLayers, bool selectedOnly)
{
    ...
    return queryClosestXYWithIndices(candidates, wcsPosition, range, draft,
                                     strictRange);
}
                                      ...
                                      bool draft, double strictRange)
{
    ...
    {
        //if (RMouseEvent::hasMouseMoved()) { return REntity::INVALID_ID; }
        QSharedPointer<REntity> e = queryEntityDirect(*it);
        if (e.isNull()) { continue; }
        double dist =
                e->getDistanceTo(wcsPosition, true, range, draft, strictRange);
...
RDocument::queryClosestXYWithIndices(QMap<REntity::Id, QSet<int>> &candidates,
                                     const RVector &wcsPosition, double range,
                                     bool draft, double strictRange)
{

    double minDist = RMAXDOUBLE;
    QPair<REntity::Id, QSet<int>> ret =
            QPair<REntity::Id, QSet<int>>(REntity::INVALID_ID, QSet<int>());
    ...
    for (it = candidates.begin(); it != candidates.end(); it++)
    {
        ...
        //{
        //    return QPair<REntity::Id, QSet<int>>(REntity::INVALID_ID,
        //                                         QSet<int>());
        //}
...
QSet<REntity::Id> RDocument::queryContainedEntities(const RBox &box) const
{
    ...
    QSet<REntity::Id> ret =
            RS::toSet<REntity::Id>(si->queryContained(box).keys());
...
QSet<REntity::Id> RDocument::queryIntersectedEntitiesXYFast(const RBox &box)
{
    ...
    if (boxExpanded.contains(getBoundingBox()))
    {
        ...
        ////::stopTimer(70, "queryAllVisibleEntities");
        return ids;
    }
...
QSet<REntity::Id>
RDocument::queryIntersectedShapesXYFast(const RBox &box,
                                        bool noInfiniteEntities)
{
    // always include construction lines (XLine):
    QSet<REntity::Id> infinites;
    if (!noInfiniteEntities) { infinites = queryInfiniteEntities(); }

    // box is completely outside the bounding box of this document:
    if (box.isOutside(getBoundingBox())) { return infinites; }

    RSpatialIndex *si = getSpatialIndexForBlock(getCurrentBlockId());
    ////::startTimer(120);
    QSet<REntity::Id> candidates =
            RS::toSet<REntity::Id>(si->queryIntersected(box).keys());
    ////::stopTimer(120, "si->queryIntersected");
    ...
    for (it = candidates.begin(); it != candidates.end(); ++it)
    {
        //if (RMouseEvent::hasMouseMoved()) { return QSet<REntity::Id>(); }
...
QSet<REntity::Id> RDocument::queryIntersectedEntitiesXY(
        const RBox &box, bool checkBoundingBoxOnly, bool includeLockedLayers,
        RBlock::Id blockId, const QList<RS::EntityType> &filter,
        bool selectedOnly, RLayer::Id layerId) const
{

    return RS::toSet<REntity::Id>(
            queryIntersectedEntitiesXYWithIndex(box, checkBoundingBoxOnly,
                                                includeLockedLayers, blockId,
                                                filter, selectedOnly, layerId)
                    .keys());
}
        ...
        bool selectedOnly, RLayer::Id layerId) const
{
    ...
    if (usingCurrentBlock && boxExpanded.contains(getBoundingBox()))
    {
        ...
        else { ids = queryAllEntities(false, false); }
    ...
    return queryIntersectedShapesXY(box, checkBoundingBoxOnly,
                                    includeLockedLayers, blockId, filter,
                                    selectedOnly, layerId);
}
        ...
        bool selectedOnly, RLayer::Id layerId) const
{
    ...
    {
        QSet<REntity::Id> ids;
        if (onlyVisible) { ids = queryAllVisibleEntities(); }
        else { ids = queryAllEntities(false, false); }
        QSet<REntity::Id>::iterator it;
        for (it = ids.begin(); it != ids.end(); it++)
        {
    ...
    for (it = candidates.begin(); it != candidates.end(); ++it)
    {
        ...
        QSharedPointer<REntity> entity;
        if (onlyVisible) { entity = queryVisibleEntityDirect(it.key()); }
        else { entity = queryEntityDirect(it.key()); }
        if (entity.isNull()) { continue; }
        ...
        if (!onlyVisible)
        {
            ...
            if (entity->getBlockId() != blockId) { continue; }
        ...
        {
            if (isLayerLocked(entity->getLayerId())) { continue; }
        }

        // restrict to a specific layer:
        if (layerId != RLayer::INVALID_ID)
        {
            if (entity->getLayerId() != layerId) { continue; }
        }
        ...
        // apply filter:
        if (filterSet.contains(entity->getType())) { continue; }
        ...
        if (!checkBoundingBoxOnly && !entity->intersectsWith(pl)) { continue; }
                                                    ...
                                                    RObject::Id layerId)
{
    ...
    do {
        ...
        for (int i = 0; i < connectionPoints.length(); ++i)
        {
            ...
            QSet<REntity::Id> candidates = queryIntersectedEntitiesXY(
                    box, true, false, RBlock::INVALID_ID,
                    QList<RS::EntityType>(), false, layerId);
            ...
            for (it = candidates.begin();
                 it != candidates.end() /*&& found<=10*/; ++it)
            {
                REntity::Id candidateId = *it;
                QSharedPointer<REntity> candidate =
                        queryEntityDirect(candidateId);
            ...
            for (int q = 0; q < connectedEntities.length(); ++q)
            {
                ...
                {
                    RVector ep = eps[ck];
                    if (connectedPoint.equalsFuzzy(ep)) { continue; }
...
RDocument::queryDocumentVariablesDirect() const
{
...
RDocument::queryObjectByHandle(RObject::Handle objectHandle) const
{
...
RDocument::queryVisibleEntityDirect(REntity::Id entityId) const
{
...
RDocument::queryLayerState(RLayerState::Id layerStateId) const
{
 ...
 */
QSharedPointer<RLayerState>
RDocument::queryLayerStateDirect(RLayerState::Id layerStateId) const
{
 ...
 */
QSharedPointer<RLayerState>
RDocument::queryLayerState(const QString &layerStateName) const
{
...
RDocument::queryBlockDirect(const QString &blockName) const
{
...
RDocument::queryLinetype(RLinetype::Id linetypeId) const
{
 ...
 */
QSharedPointer<RLinetype>
RDocument::queryLinetype(const QString &linetypeName) const
{
 ...
 */
void RDocument::selectEntity(REntity::Id entityId, bool add,
 ...
 *
 */
int RDocument::selectEntities(const QSet<REntity::Id> &entityIds, bool add,
                              QSet<REntity::Id> *affectedEntities)
{
 ...
 */
bool RDocument::hasSelection() const { return storage.hasSelection(); }
 ...
 */
RBox RDocument::getSelectionBox() const { return storage.getSelectionBox(); }
...
void RDocument::clearSpatialIndices()
{
    ...
    for (it = spatialIndicesByBlock.begin(); it != spatialIndicesByBlock.end();
         it++)
    {
        delete *it;
    }
...
void RDocument::rebuildSpatialIndex()
{
    ...
    if (!disableSpatialIndicesByBlock)
    {
        ...
        for (int i = 0; i < blockIds.length(); i++)
        {
            ...
            for (int i = allIdsByBlock[blockId].length() - 1;
                 i >= 0 && !allIdsByBlock[blockId].isEmpty(); i--)
            {
    ...
    else { spatialIndex.bulkLoad(allIds, allBbs); }
...
void RDocument::removeBlockFromSpatialIndex(RBlock::Id blockId)
{
    ...
    for (it = blockRefIds.begin(); it != blockRefIds.end(); it++)
    {
        QSharedPointer<RBlockReferenceEntity> blockRef =
                queryEntityDirect(*it).dynamicCast<RBlockReferenceEntity>();
...
{
    if (blockId == referencedBlockId) { return true; }
    ...
    for (it = blockEntityIds.begin(); it != blockEntityIds.end(); it++)
    {
        ...
        QSharedPointer<RBlockReferenceEntity> blockReference =
                entity.dynamicCast<RBlockReferenceEntity>();
        if (blockReference.isNull()) { continue; }

        if (blockContainsReferences(blockReference->getReferencedBlockId(),
                                    referencedBlockId))
        {
            recursionDepth--;
            return true;
                                       ...
                                       const QList<RBox> &boundingBoxes)
{
    ...
    if (bbs.isEmpty()) { bbs = entity->getBoundingBoxes(); }
 ...
 */
QList<RTransaction> RDocument::undo() { return transactionStack.undo(); }
 ...
 *   this set to update scenes, views, etc accordingly.
 */
QList<RTransaction> RDocument::redo() { return transactionStack.redo(); }

/**
 * \copydoc RStorage::isModified
 */
bool RDocument::isModified() const { return storage.isModified(); }
 ...
 */
QString RDocument::getLastModified() const { return storage.getLastModified(); }
...
void RDocument::copyVariablesFrom(const RDocument &other)
{
    RTransaction *transaction = new RTransaction(
            storage, "Copy variables from other document", false);

    bool useLocalTransaction;
    QSharedPointer<RDocumentVariables> docVars =
            storage.startDocumentVariablesTransaction(transaction,
                                                      useLocalTransaction);

    for (RS::KnownVariable kv = RS::ANGBASE; kv <= RS::MaxKnownVariable;
         kv = (RS::KnownVariable)(kv + 1))
    {
    ...
    storage.endDocumentVariablesTransaction(transaction, useLocalTransaction,
                                            docVars);
    ...
    for (int i = 0; i < keys.length(); i++)
    {
        ...
        if (otherV.isValid()) { setVariable(key, otherV); }
    }
...
{
    RTransaction *transaction =
            new RTransaction(storage, "Add auto variable", false);
    bool useLocalTransaction;
    QSharedPointer<RDocumentVariables> docVars =
            storage.startDocumentVariablesTransaction(transaction,
                                                      useLocalTransaction);
...
QStringList RDocument::getAutoVariables() const
{
    ...
    if (docVars.isNull()) { return QStringList(); }
...
QString RDocument::substituteAutoVariables(const QString &expression)
{
    ...
    QSharedPointer<RDocumentVariables> docVars = queryDocumentVariablesDirect();
    if (docVars.isNull()) { return expression; }
    ...
    for (int i = 0; i < autoVariables.length(); i++)
    {
        ...
        double value =
                docVars->getCustomDoubleProperty("QCAD", key, RNANDOUBLE);
        if (RMath::isNaN(value)) { continue; }
        exp = exp.replace(QRegularExpression(QString("\\b%1\\b").arg(key)),
                          QString("%1").arg(value, 0, 'f', 12));
    }
...
{
    QSharedPointer<RDocumentVariables> docVars = queryDocumentVariablesDirect();
    return docVars->hasCustomProperty("QCAD", "WorkingSet/BlockName") &&
           !docVars->hasCustomProperty("QCAD", "WorkingSet/Ignore");
}
...
void RDocument::setIgnoreWorkingSet(bool on)
{
    if (on)
    {
        queryDocumentVariablesDirect()->setCustomProperty(
                "QCAD", "WorkingSet/Ignore", true);
    }
    else
    {
        queryDocumentVariablesDirect()->removeCustomProperty(
                "QCAD", "WorkingSet/Ignore");
    }
}
