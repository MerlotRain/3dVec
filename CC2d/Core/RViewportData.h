...
public:
    ...
    virtual RS::EntityType getType() const { return RS::EntityViewport; }

    bool isValid() const { return RPoint::isValid(); }
    ...
    virtual void setZ(double z) { RPoint::setZ(z); }
    virtual QList<RVector>
    ...
    virtual bool move(const RVector &offset) { return RPoint::move(offset); }
    virtual bool rotate(double rotation,
                        const RVector &center = RDEFAULT_RVECTOR)
    {
        return RPoint::rotate(rotation, center);
    }
    virtual bool mirror(const RLine &axis) { return RPoint::mirror(axis); }
    ...
    virtual RBox getBoundingBox() const { return getBoundingBox(false); }

    virtual RShape *castToShape() { return this; }

    int getViewportId() const { return viewportId; }

    void setViewportId(int id) { viewportId = id; }

    int getStatus() const { return status; }

    void setStatus(int s) { status = s; }

    RVector getCenter() const { return position; }

    void setCenter(const RVector &c) { position = c; }

    RVector getViewCenter() const { return viewCenter; }

    void setViewCenter(const RVector &c) { viewCenter = c; }

    RVector getViewTarget() const { return viewTarget; }

    void setViewTarget(const RVector &t) { viewTarget = t; }
    ...
    bool isOverall() const { return overall; }

    void setOverall(bool on) { overall = on; }

    bool isOff() const { return testStatusFlag(Off); }

    void setOff(bool v) { setStatusFlag(Off, v); }
...
protected:
    ...
    bool testStatusFlag(Status s) const { return (status & s) == s; }
