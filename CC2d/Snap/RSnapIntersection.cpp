                                ...
                                const RBox &queryBox)
{
    ...
    for (it1 = candidates.begin(); it1 != candidates.end(); it1++)
    {
        ...
        if (!layer1->isSnappable() ||
            !document->isParentLayerSnappable(*layer1))
        {
            continue;
        }
        ...
        for (it2 = it1; it2 != candidates.end(); it2++)
        {
            ...
            QSharedPointer<RLayer> layer2 =
                    document->queryLayerDirect(layer2Id);
            if (!layer2->isSnappable() ||
                !document->isParentLayerSnappable(*layer2))
            {
                continue;
            }
            ...
            QList<RVector> ipsCandidates = e1->getIntersectionPoints(
                    *e2, true, queryBox, true, &subEntityIdsCandidates);

            if (ipsCandidates.isEmpty()) { continue; }
            ...
            QPair<REntity::Id, REntity::Id> subEntityIds(REntity::INVALID_ID,
                                                         REntity::INVALID_ID);
