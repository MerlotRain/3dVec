...
RLinetypePattern::RLinetypePattern(bool metric, const QString &name,
                                   const QString &description, int num...)
      ...
      noOffset(false)
{
    ...
    va_list vl;
    va_start(vl, num);
    for (int i = 0; i < num; ++i) { dashes.append(va_arg(vl, double)); }
    va_end(vl);
...
void RLinetypePattern::set(const QList<double> &dashes)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        for (int a = 1; a < num; ++a)
        {
            if (fabs(normalizedPattern[RMath::absmod((i - a), num)] -
                     normalizedPattern[RMath::absmod((i + a), num)]) > 0.1)
            {
                sym = false;
                break;
            }
...
bool RLinetypePattern::operator==(const RLinetypePattern &other) const
{
    ...
    if (name.toLower() != other.name.toLower()) { return false; }
...
void RLinetypePattern::scale(double factor)
{
    ...
    {
        pattern[i] *= factor;
        if (shapeScales.contains(i)) { shapeScales[i] *= factor; }
        else { shapeScales.insert(i, factor); }
        if (shapeOffsets.contains(i)) { shapeOffsets[i] *= factor; }
    }
    for (int i = 0; i < symmetries.length(); ++i) { symmetries[i] *= factor; }
    updateShapes();
    patternString = "";
}
 ...
 */
double RLinetypePattern::getPatternOffsetAt(double length, double symmetryPos,
                                            double *gap, bool end)
{
    Q_UNUSED(end)

    double patternLength = getPatternLength();
    if (patternLength < RS::PointTolerance) { return 0.0; }

    double offset = length / 2 - symmetryPos;
    int m = (int) RMath::trunc(offset / patternLength);
    offset -= (m + 1) * patternLength;
    if (gap != NULL) { *gap = getDelta(-offset); }
    return offset;
}
...
double RLinetypePattern::getDelta(double pos) const
{
    ...
    if (pos < 0) { pos += (RMath::trunc(pos / l) + 1) * l; }
    else if (pos > l) { pos -= RMath::trunc(pos / l) * l; }
    double total = 0.0;
    for (int i = 0; i < pattern.length(); ++i)
    {
        ...
        if (total >= pos)
        {
            if (pattern[i] < 0) { return -(total - pos); }
            return total - pos;
        }
    }
...
bool RLinetypePattern::hasDashAt(double pos) const
{
    if (pos < 0)
    {
        pos += (RMath::trunc(pos / getPatternLength()) + 1) *
               getPatternLength();
    }
    ...
    {
        total += fabs(pattern[i]);
        if (total > pos) { return pattern[i] > 0; }
    }
    qWarning("RLinetypePattern::hasDashAt: invalid pos argument");
    return false;
}

bool RLinetypePattern::isValid() const { return pattern.length() > 0; }

int RLinetypePattern::getNumDashes() const { return pattern.length(); }

QString RLinetypePattern::getName() const { return name; }

void RLinetypePattern::setName(const QString &n) { name = n; }

QString RLinetypePattern::getDescription() const { return description; }

void RLinetypePattern::setDescription(const QString &d) { description = d; }

double RLinetypePattern::getScreenScale() const { return screenScale; }

void RLinetypePattern::setScreenScale(double s) { screenScale = s; }

bool RLinetypePattern::getNoOffset() const { return noOffset; }

void RLinetypePattern::setNoOffset(bool n) { noOffset = n; }
...
QString RLinetypePattern::getLabel() const
{
    ...
    if (nameMap.isEmpty()) { initNameMap(); }

    QString nameUpper = name.toUpper();
    if (nameMap.contains(nameUpper)) { return nameMap.value(nameUpper); }
    ...
    for (int i = 0; i < parts.length(); i++)
    {
        ...
        if (part.startsWith("[", Qt::CaseInsensitive))
        {
            ...
            for (int k = 3; k + 1 <= rx.captureCount(); k += 2)
            {
                ...
                if (c == "S") { shapeScales.insert(idx, val); }
                if (c == "R")
                {
                ...
                if (c == "X")
                {
                    ...
                    else { shapeOffsets.insert(idx, RVector(val, 0)); }
                }
                if (c == "Y")
                {
                    if (shapeOffsets.contains(idx))
                    ...
                    else { shapeOffsets.insert(idx, RVector(0, val)); }
                }
            }
        }
        else { dashes.append(part.toDouble()); }
    }
...
QString RLinetypePattern::getPatternString() const
{
    if (patternString.isEmpty())
    {
        ...
        for (int i = 0; i < getNumDashes(); i++)
        {
            ...
            if (shapeNumber == 0 && text.isEmpty()) { continue; }
            ...
            if (shapeNumber != 0)
            {
                ...
                buffer += ",";
                if (!textStyle.isEmpty()) { buffer += textStyle; }
                else { buffer += "NULL style"; }
            }
            else
            {
                buffer += ",[\"";
            ...
            {
                buffer += ",R=";
                buffer += QString("%1").arg(
                        RMath::rad2deg(getShapeRotationAt(i)));
            }
...
QList<double> RLinetypePattern::getPattern() const { return pattern; }

double RLinetypePattern::getPatternLength() const
{
    double ret = 0.0;
    for (int i = 0; i < pattern.length(); ++i) { ret += fabs(pattern[i]); }
    return ret;
}
                                         ...
                                         int i) const
{
    ...
    if (i < 0 || i >= dashes.length()) { return ret; }

    for (int k = 0; k < i; k++) { ret += fabs(dashes[k]); }
    return ret;
}
...
{
    if (!shapeNumbers.contains(i)) { return 0; }
...
{
    if (!shapeScales.contains(i)) { return 0.0; }
...
{
    if (!shapeRotations.contains(i)) { return 0.0; }
...
{
    if (!shapeOffsets.contains(i)) { return RVector(0, 0); }
...
{
    if (!shapeTexts.contains(i)) { return QString(); }
    return shapeTexts[i];
}
...
{
    if (!shapeTextStyles.contains(i)) { return QString(); }
...
bool RLinetypePattern::hasShapes() const { return !shapes.isEmpty(); }

bool RLinetypePattern::hasShapeAt(int i) const { return shapes.contains(i); }

QList<int> RLinetypePattern::getShapeIndices() const { return shapes.keys(); }

QList<QPainterPath> RLinetypePattern::getShapeAt(int i) const
{
