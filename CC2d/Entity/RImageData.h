 ...
 * \ingroup entity
 */
class CC2DCORE_EXPORT RImageData : public REntityData
{
...
public:
    ...
    RImageData(const QString &fileName, const RVector &insertionPoint,
               const RVector &uVector, const RVector &vVector, int brightness,
               int contrast, int fade);
    virtual ~RImageData();

    virtual RS::EntityType getType() const { return RS::EntityImage; }
    ...
    RVector getInsertionPoint() const { return insertionPoint; }
    void setInsertionPoint(const RVector &ip) { insertionPoint = ip; }
    double getAngle() const { return uVector.getAngle(); }
    void setAngle(double a)
    {
        uVector.setAngle(a);
        vVector.setAngle(a + M_PI / 2.0);
    }
    RVector getUVector() const { return uVector; }
    void setUVector(const RVector &v) { uVector = v; }
    RVector getVVector() const { return vVector; }
    void setVVector(const RVector &v) { vVector = v; }
    ...
    void setContrast(int v) { contrast = v; }
    int getFade() const { return fade; }
    void setFade(int v) { fade = v; }
