...
public:
    ...
    RHatchData(bool solid, double scaleFactor, double angle,
               const QString &patternName);

    virtual RS::EntityType getType() const { return RS::EntityHatch; }
    ...
    RPattern getCustomPattern() const { return pattern; }

    void setCustomPattern(const RPattern &p) { pattern = p; }
    ...
    RVector getOriginPoint() const { return originPoint; }

    void setOriginPoint(const RVector &op, bool clearCustom = true)
    {
        originPoint = op;
        if (clearCustom) { clearCustomPattern(); }
    }
    ...
    int getLoopCount() const { return boundary.count(); }
    ...
    void autoCloseLoops()
    {
        if (hatchProxy != NULL) { hatchProxy->autoCloseLoops(*this); }
    }

    int getComplexity() const;

    void setPattern(const RPattern &p) { pattern = p; }
    ...
    static bool hasProxy() { return hatchProxy != NULL; }
    ...
    {
        if (hatchProxy != NULL) { delete hatchProxy; }
        hatchProxy = p;
    }
     ...
     * \nonscriptable
     */
    static RHatchProxy *getHatchProxy() { return hatchProxy; }
