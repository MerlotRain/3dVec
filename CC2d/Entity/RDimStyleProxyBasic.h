...
public:
    RDimStyleProxyBasic()
        : document(NULL), dimensionData(NULL), dimStyle(NULL), preview(false),
          forceSelected(false), outsideArrow1(false), outsideArrow2(false)
    {
    }

    virtual ~RDimStyleProxyBasic() {}

    virtual void render(const RDimStyle &dimStyle, const REntityData &entity,
                        bool preview, bool forceSelected);

protected:
    virtual void renderPrivate(const RDimStyle &dimStyle,
                               const REntityData &entity, bool preview,
                               bool forceSelected);
    ...
    virtual void breakUpDimArc(QList<QSharedPointer<RShape>> &shapes,
    ...
    virtual RPolyline getTextBox(const RDimensionData &dimensionData,
    ...
    virtual void updateTextPosition(const QString &text, double textWidth,
                                    const RVector &dimLine1,
                                    const RVector &dimLine2, bool corrected,
    ...
    getDimensionLineShapes(const RVector &p1, const RVector &p2, bool arrow1,
                           bool arrow2, double angleHint = RNANDOUBLE) const;
    virtual bool hasSpaceForArrows(double dimLineLength) const;
    virtual QList<QSharedPointer<RShape>> getArrow(const RVector &position,
                                                   double direction) const;
    virtual void adjustExtensionLineFixLength(RLine &extLine1, RLine &extLine2,
