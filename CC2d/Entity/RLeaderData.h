...
public:
    ...
    virtual RS::EntityType getType() const { return RS::EntityLeader; }
    ...
    double getDimasz(bool scale = true) const
    {
        ...
        if (dimasz > 0.0) { v = dimasz; }

        else if (document != NULL)
        {
            QSharedPointer<RDimStyle> dimStyle =
                    document->queryDimStyleDirect();
            if (!dimStyle.isNull())
            {
    ...
    {
        // get value from override:
        if (dimscale > 0.0) { return dimscale; }
    ...
    bool hasArrowHead() const { return arrowHead; }
    ...
    int countVertices() const { return RPolyline::countVertices(); }
    ...
    bool isSplineShaped() const { return splineShaped; }

    void setSplineShaped(bool on) { splineShaped = on; }
