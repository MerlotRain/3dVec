...
public:
    ...
    virtual RS::EntityType getType() const { return RS::EntitySpline; }
    ...
    virtual void setZ(double z) { RSpline::setZ(z); }
    virtual void to2D() { RSpline::to2D(); }
    virtual QList<RVector>
    ...
    {
        return REntityData::getIntersectionPoints(
                other, limited, same, queryBox, ignoreComplex, entityIds);
    }
    virtual QList<RVector>
    ...
    virtual bool move(const RVector &offset) { return RSpline::move(offset); }
    ...
    virtual bool mirror(const RLine &axis) { return RSpline::mirror(axis); }
    ...
    void setShape(const RSpline &s) { copySpline(s); }
    ...
    void setPeriodic(bool on) { RSpline::setPeriodic(on); }

    bool isValid() const { return RSpline::isValid(); }

    int getDegree() const { return RSpline::getDegree(); }

    double getDirection1() const { return RSpline::getDirection1(); }

    double getDirection2() const { return RSpline::getDirection2(); }

    RVector getStartPoint() const { return RSpline::getStartPoint(); }

    RVector getEndPoint() const { return RSpline::getEndPoint(); }

    void setStartPoint(const RVector &sp) { RSpline::setStartPoint(sp); }

    void setEndPoint(const RVector &ep) { RSpline::setEndPoint(ep); }

    bool reverse() { return RSpline::reverse(); }
    ...
    bool isClosed() const { return RSpline::isClosed(); }

    bool isPeriodic() const { return RSpline::isPeriodic(); }

    RVector getTangentAtStart() const { return RSpline::getTangentAtStart(); }

    RVector getTangentAtEnd() const { return RSpline::getTangentAtEnd(); }
    ...
    QList<RVector> getFitPoints() const { return RSpline::getFitPoints(); }

    QList<double> getKnotVector() const { return RSpline::getKnotVector(); }
    ...
    QList<double> getWeights() const { return RSpline::getWeights(); }

    double getLength() const { return RSpline::getLength(); }
    RVector getPointAt(double t) const { return RSpline::getPointAt(t); }
