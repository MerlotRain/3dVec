 ...
 * \ingroup entity
 */
class CC2DCORE_EXPORT RHatchEntity : public REntity
{
public:
...
public:
    ...
    static RS::EntityType getRtti() { return RS::EntityHatch; }
    ...
    void setData(RHatchData &d) { data = d; }

    bool hasCustomPattern() const { return data.hasCustomPattern(); }

    RPattern getCustomPattern() const { return data.getCustomPattern(); }

    void setCustomPattern(const RPattern &p) { data.setCustomPattern(p); }

    virtual bool setProperty(RPropertyTypeId propertyTypeId,
                             const QVariant &value,
                             RTransaction *transaction = NULL);
    bool setBoundaryVector(RObject::XYZ xyz, const QVariant &value,
                           bool condition);
    ...
    void newLoop() { data.newLoop(); }

    void addBoundary(QSharedPointer<RShape> shape) { data.addBoundary(shape); }

    int getLoopCount() const { return data.getLoopCount(); }

    bool isSolid() const { return data.isSolid(); }

    void setSolid(bool on) { data.setSolid(on); }
    ...
    void setAngle(double a) { data.setAngle(a); }

    RVector getOriginPoint() const { return data.getOriginPoint(); }

    void setOriginPoint(const RVector &op) { data.setOriginPoint(op); }

    QString getPatternName() const { return data.getPatternName(); }

    void setPatternName(const QString &n) { data.setPatternName(n); }

    int getTransparency() const { return data.getTransparency(); }

    void setTransparency(int t) { data.setTransparency(t); }

    void clearCustomPattern() { data.clearCustomPattern(); }
    ...
    QList<RPolyline>
    getBoundaryAsPolylines(double segmentLength = RDEFAULT_MIN1) const
    {
