...
public:
    RAttributeData() {}
    RAttributeData(const RTextBasedData &textData,
                   RBlockReferenceEntity::Id blockReferenceId,
                   const QString &tag);
    virtual ~RAttributeData() {}

    virtual RS::EntityType getType() const { return RS::EntityAttribute; }
    ...
    void setInvisible(bool i) { invisible = i; }
    ...
    virtual RColor getColor() const { return RTextBasedData::getColor(); }

    virtual QList<RRefPoint>
    getReferencePoints(RS::ProjectionRenderingHint hint) const;
