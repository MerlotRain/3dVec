...
RDimAlignedData::RDimAlignedData(RDocument *document,
...
bool RDimAlignedData::isValid() const
{
    return RDimensionData::isValid() &&
           !extensionPoint1.equalsFuzzy(extensionPoint2);
}

QList<RRefPoint>
RDimAlignedData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
    Q_UNUSED(hint)
...
void RDimAlignedData::recomputeDefinitionPoint(const RVector &oldExtPoint1,
                                               ...
                                               const RVector &newExtPoint2)
{
    ...
    if (s == RS::LeftHand) { a += M_PI / 2.0; }
    else { a -= M_PI / 2.0; }
    RVector v = RVector::createPolar(d, a);
    RVector dp = newExtPoint1 + v;
    if (dp.isValid()) { definitionPoint = dp; }
}
