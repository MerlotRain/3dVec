...
public:
    ...
    RDimensionData(const RVector &definitionPoint,
                   const RVector &textPositionCenter, RS::VAlign valign,
                   RS::HAlign halign, RS::TextLineSpacingStyle lineSpacingStyle,
                   double lineSpacingFactor, const QString &text,
                   const QString &fontName, double textRotation);
    ...
    virtual RS::EntityType getType() const { return RS::EntityDimension; }
    virtual RBox getBoundingBox(bool ignoreEmpty = false) const;

    void render() const;

    void updateBoundingBox(const RBox &box) const { boundingBox = box; }
    ...
    void updateTextData(const RTextData &d) const { textData = d; }
    void updateShapes(const QList<QSharedPointer<RShape>> &s) const
    {
        shapes = s;
    }
    void updateArrowPos1(const RVector &p) const { arrow1Pos = p; }
    void updateArrowPos2(const RVector &p) const { arrow2Pos = p; }
    ...
    RVector getDefinitionPoint() const { return definitionPoint; }
    ...
    QString getUpperTolerance() const { return upperTolerance; }
    ...
    QString getLowerTolerance() const { return lowerTolerance; }

    void setTextPosition(const RVector &p)
    {
        if (p.isSane()) { textPositionCenter = p; }
    ...
    {
        if (textPositionSide.isValid()) { return textPositionSide; }
        else { return textPositionCenter; }
    }
    ...
    QString getDimBlockName() const { return dimBlockName; }

    bool hasOverrides() const { return overrides.isValid(); }
    ...
    RDimStyleData getOverrides() const { return overrides; }
    ...
    double getDimlfac() const { return getDimXDouble(RS::DIMLFAC); }

    void setDimlfac(double f) { setDimXDouble(RS::DIMLFAC, f); }

    double getLinearFactor() const { return getDimlfac(); }
    void setLinearFactor(double f) { setDimlfac(f); }

    double getDimscale() const { return getDimXDouble(RS::DIMSCALE); }

    void setDimscale(double f) { setDimXDouble(RS::DIMSCALE, f); }

    bool isArrow1Flipped() const { return arrow1Flipped; }
    void setArrow1Flipped(bool on) { arrow1Flipped = on; }
    bool isArrow2Flipped() const { return arrow2Flipped; }
    void setArrow2Flipped(bool on) { arrow2Flipped = on; }

    bool isExtLineFix() const { return extLineFix; }
    void setExtLineFix(bool on) { extLineFix = on; }

    double getExtLineFixLength() const { return extLineFixLength; }
    void setExtLineFixLength(double v) { extLineFixLength = v; }

    void adjustExtensionLineFixLength(RLine &extLine1, RLine &extLine2,
    ...
    void setDimexo(double v) { setDimXDouble(RS::DIMEXO, v); }
    double getDimexe() const
    {
        return getDimXDouble(RS::DIMEXE) * getDimscale();
    }
    void setDimexe(double v) { setDimXDouble(RS::DIMEXE, v); }
    double getDimasz() const
    {
        return getDimXDouble(RS::DIMASZ) * getDimscale();
    }
    void setDimasz(double v) { setDimXDouble(RS::DIMASZ, v); }
    double getDimdli() const
    {
        return getDimXDouble(RS::DIMDLI) * getDimscale();
    }
    void setDimdli(double v) { setDimXDouble(RS::DIMDLI, v); }
    double getDimgap() const
    {
        return getDimXDouble(RS::DIMGAP) * getDimscale();
    }
    void setDimgap(double v) { setDimXDouble(RS::DIMGAP, v); }
    double getDimtxt() const
    {
        return getDimXDouble(RS::DIMTXT) * getDimscale();
    }
    void setDimtxt(double t) { setDimXDouble(RS::DIMTXT, t); }
    int getDimlunit() const { return getDimXInt(RS::DIMLUNIT); }
    ...
    int getDimjust() const { return getDimXInt(RS::DIMJUST); }
    /**
     * \return 0 for text above dimension line, otherwise text on dimension line.
     */
    int getDimtad() const { return getDimXInt(RS::DIMTAD); }
    void setDimtad(int v) { setDimXInt(RS::DIMTAD, v); }
    /**
     * \return 0 for aligned, other for horizontal text label.
     */
    bool getDimtih() const { return getDimXBool(RS::DIMTIH); }
    void setDimtih(bool v) { setDimXBool(RS::DIMTIH, v); }
    double getDimtsz() const
    {
        return getDimXDouble(RS::DIMTSZ) * getDimscale();
    }
    void setDimtsz(double t) { setDimXDouble(RS::DIMTSZ, t); }
    int getDimzin() const { return getDimXInt(RS::DIMZIN); }
    void setDimzin(int v) { setDimXInt(RS::DIMZIN, v); }
    int getDimaunit() const { return getDimXInt(RS::DIMAUNIT); }
    void setDimaunit(int v) { setDimXInt(RS::DIMAUNIT, v); }
    int getDimadec() const { return getDimXInt(RS::DIMADEC); }
    void setDimadec(int v) { setDimXInt(RS::DIMADEC, v); }
    int getDimdec() const { return getDimXInt(RS::DIMDEC); }
    void setDimdec(int v) { setDimXInt(RS::DIMDEC, v); }
    int getDimazin() const { return getDimXInt(RS::DIMAZIN); }
    void setDimazin(int v) { setDimXInt(RS::DIMAZIN, v); }
    int getDimdsep() const { return getDimXInt(RS::DIMDSEP); }
    void setDimdsep(int v) { setDimXInt(RS::DIMDSEP, v); }
    RColor getDimclrt() const { return getDimXColor(RS::DIMCLRT); }
    void setDimclrt(const RColor &v) { setDimXColor(RS::DIMCLRT, v); }

    bool useArchTick() const { return getDimXDouble(RS::DIMTSZ) > 0.0; }

    int getDimblk() const { return getDimXInt(RS::DIMBLK); }
    ...
    double getLineSpacingFactor() const { return lineSpacingFactor; }

    double getTextRotation() const { return textRotation; }

    void setTextRotation(double r) { textRotation = r; }
