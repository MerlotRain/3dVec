...
{
    return RDimensionData::isValid() && getExtensionLine1End().isValid() &&
           getExtensionLine2End().isValid() && getDimArcPosition().isValid();
}

bool RDimAngularData::isSane() const
{
    return RDimensionData::isSane() && getExtensionLine1End().isSane() &&
           getExtensionLine2End().isSane() && getDimArcPosition().isSane();
}

QList<RRefPoint>
RDimAngularData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
...
double RDimAngularData::getMeasuredValue() const { return getAngle(); }
