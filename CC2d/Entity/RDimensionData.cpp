...
RDimensionData::RDimensionData(RDocument *document)
    : REntityData(document), textPositionCenter(RVector::invalid),
      textPositionSide(RVector::invalid), valign(RS::VAlignMiddle),
      halign(RS::HAlignMid), lineSpacingStyle(RS::Exact),
      lineSpacingFactor(1.0), defaultAngle(RNANDOUBLE), textRotation(0.0),
      overrides(true),
      //    linearFactor(1.0),
      //    dimScaleOverride(0.0),
      //    dimtxtOverride(0.0),
      //    dimlunitOverride(-1),
      arrow1Flipped(false), arrow2Flipped(false), extLineFix(false),
      extLineFixLength(0.0), dirty(true), dimLineLength(0.0),
      arrow1Pos(RVector::invalid), arrow2Pos(RVector::invalid),
      autoTextPos(true)
{
}
...
RDimensionData::RDimensionData(const RVector &definitionPoint,
                               const RVector &textPosition, RS::VAlign valign,
                               RS::HAlign halign,
                               RS::TextLineSpacingStyle lineSpacingStyle,
                               double lineSpacingFactor, const QString &text,
                               const QString &style, double textRotation)
    : definitionPoint(definitionPoint), textPositionCenter(textPosition),
      textPositionSide(RVector::invalid), valign(valign), halign(halign),
      lineSpacingStyle(lineSpacingStyle), lineSpacingFactor(lineSpacingFactor),
      text(text), fontName(style), defaultAngle(RNANDOUBLE),
      textRotation(textRotation), overrides(true),
      //linearFactor(1.0),
      //dimScaleOverride(0.0),
      //dimtxtOverride(0.0),
      //dimlunitOverride(-1),
...
QVariant RDimensionData::getDimXVariant(RS::KnownVariable key) const
{
    ...
    if (hasOverride(key)) { return getVariantOverride(key); }
    ...
    else { qWarning() << "RDimensionData::getDimXVariant: no document"; }
...
QVariant RDimensionData::getVariantOverride(RS::KnownVariable key) const
{
    ...
    if (hasOverrides()) { return overrides.getVariant(key); }
    else
    {
        if (document != NULL) { return document->getKnownVariable(key, def); }
    }
    return def;
}
...
double RDimensionData::getDimXDouble(RS::KnownVariable key) const
{
    ...
    // get value from override:
    if (hasOverride(key)) { return getDoubleOverride(key); }
    ...
    else { qWarning() << "RDimensionData::getDimXDouble: no document"; }
...
double RDimensionData::getDoubleOverride(RS::KnownVariable key) const
{
    ...
    if (hasOverrides()) { return overrides.getDouble(key); }
    else
    {
        if (document != NULL)
        {
...
int RDimensionData::getDimXInt(RS::KnownVariable key) const
{
    ...
    else { qWarning() << "RDimensionData::getDimXInt: no document"; }
...
int RDimensionData::getIntOverride(RS::KnownVariable key) const
{
    ...
    if (hasOverrides()) { return overrides.getInt(key); }
    else
    {
        if (document != NULL)
        {
...
bool RDimensionData::getDimXBool(RS::KnownVariable key) const
{
    ...
    // get value from override:
    if (hasOverride(key)) { return getBoolOverride(key); }
    ...
    else { qWarning() << "RDimensionData::getDimXBool: no document"; }
...
bool RDimensionData::getBoolOverride(RS::KnownVariable key) const
{
    ...
    if (hasOverrides()) { return overrides.getBool(key); }
    else
    {
        if (document != NULL)
        {
...
RColor RDimensionData::getDimXColor(RS::KnownVariable key) const
{
    ...
    // get value from override:
    if (hasOverride(key)) { return getColorOverride(key); }
    ...
    else { qWarning() << "RDimensionData::getDimXColor: no document"; }
...
RColor RDimensionData::getColorOverride(RS::KnownVariable key) const
{
    ...
    if (hasOverrides()) { return overrides.getColor(key); }
    else
    {
        if (document != NULL)
        {
...
RVector RDimensionData::getPointOnEntity() const
{
    ...
    if (shapes.isEmpty()) { return RVector::invalid; }
...
double RDimensionData::getDistanceTo(const RVector &point, bool limited,
                                     ...
                                     double strictRange) const
{
    ...
    getTextData();
    double ret2 =
            textData.getDistanceTo(point, limited, range, draft, strictRange);
    if (RMath::isNaN(ret2)) { return ret1; }
    return qMin(ret1, ret2);
}

bool RDimensionData::intersectsWith(const RShape &shape) const
{
    if (REntityData::intersectsWith(shape)) { return true; }
    getTextData();
    if (textData.intersectsWith(shape)) { return true; }
    return false;
}

QList<RRefPoint>
RDimensionData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
    Q_UNUSED(hint)
...
bool RDimensionData::move(const RVector &offset)
{
    ...
    if (textPositionSide.isValid()) { textPositionSide.move(offset); }
    update();
    return true;
}
...
void RDimensionData::scaleVisualProperties(double scaleFactor)
{
    ...
    if (!RMath::fuzzyCompare(scaleFactor, 0.0) &&
        !RMath::fuzzyCompare(scaleFactor, 1.0))
    {
...
bool RDimensionData::mirror(const RLine &axis)
{
    ...
    if (textPositionSide.isValid()) { textPositionSide.mirror(axis); }
    update();
    return true;
}

bool RDimensionData::hasCustomTextPosition() const { return !autoTextPos; }

void RDimensionData::setCustomTextPosition(bool on) { autoTextPos = !on; }
...
RTextData &RDimensionData::initTextData() const
{
    ...
    textData = RTextData(
            RVector(0, 0),// position
            RVector(0, 0),// alignment point
            dimtxt,       // text height
            0.0, RS::VAlignMiddle, RS::HAlignCenter, RS::LeftToRight, RS::Exact,
            1.0, label,
            //fontName,
            (document == NULL || document->getDimensionFont().isEmpty())
                    ? "Standard"
                    : document->getDimensionFont(),
            false, false, 0.0,
    ...
    QSharedPointer<RDimStyle> dimStyle = document->queryDimStyleDirect();
    if (!dimStyle.isNull()) { textColor = dimStyle->getColor(RS::DIMCLRT); }
    if (textColor != RColor::ByBlock) { textData.setColor(textColor); }
...
QString RDimensionData::getMeasurement(bool resolveAutoMeasurement) const
{
    ...
    {
        if (resolveAutoMeasurement) { return getAutoLabel(); }
        else { return ""; }
    }
    ...
    {
        if (resolveAutoMeasurement) { return ""; }
        else { return "."; }
    }
    ...
    QString measurement;
    if (resolveAutoMeasurement) { measurement = getAutoLabel(); }
    else { measurement = "<>"; }

    if (text.isEmpty()) { return alignment + measurement + tolerance; }
    else
    {
        QString main = text;
...
QString RDimensionData::formatLabel(double distance) const
{
    ...
    if (document != NULL)
    {
        ret = RUnit::formatLinear(
                distance, document->getUnit(), (RS::LinearFormat) getDimlunit(),
                getDimdec(), false, (getDimzin() & 4) != 4,
                (getDimzin() & 8) != 8, false, (char) getDimdsep());
    }
    else { ret = QString("%1").arg(distance); }
...
QString RDimensionData::formatAngleLabel(double angle) const
{
    ...
    if (document != NULL)
    {
        ret = RUnit::formatAngle(angle, (RS::AngleFormat) getDimaunit(),
                                 getDimadec(), (getDimazin() & 1) != 1,
                                 (getDimazin() & 2) != 2, (char) getDimdsep());
    }
    else { ret = QString("%1").arg(angle); }
...
bool RDimensionData::hasDimensionBlockReference() const
{
    ...
    if (dimBlockName.isEmpty()) { return false; }
    ...
    if (!doc->hasBlockEntities(dimBlockId)) { return false; }

    return true;
}

QSharedPointer<RBlockReferenceEntity>
RDimensionData::getDimensionBlockReference() const
{
    QString dimBlockName = getDimBlockName();
    ...
    RBlockReferenceEntity *dimBlockReference = new RBlockReferenceEntity(
            (RDocument *) doc,
            RBlockReferenceData(dimBlockId, RVector(0, 0), RVector(1, 1),
                                0.0) /*, getId()*/);
