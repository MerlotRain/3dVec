...
RToleranceEntity::RToleranceEntity(RDocument *document,
...
RToleranceEntity::RToleranceEntity(const RToleranceEntity &other)
    : REntity(other)
{
    //RDebug::incCounter("RToleranceEntity");
    data = other.data;
...
{
    RToleranceEntity::PropertyCustom.generateId(RToleranceEntity::getRtti(),
                                                RObject::PropertyCustom);
    RToleranceEntity::PropertyHandle.generateId(RToleranceEntity::getRtti(),
                                                RObject::PropertyHandle);
    RToleranceEntity::PropertyProtected.generateId(RToleranceEntity::getRtti(),
                                                   RObject::PropertyProtected);
    RToleranceEntity::PropertyWorkingSet.generateId(
            RToleranceEntity::getRtti(), RObject::PropertyWorkingSet);
    RToleranceEntity::PropertyType.generateId(RToleranceEntity::getRtti(),
                                              REntity::PropertyType);
    RToleranceEntity::PropertyBlock.generateId(RToleranceEntity::getRtti(),
                                               REntity::PropertyBlock);
    RToleranceEntity::PropertyLayer.generateId(RToleranceEntity::getRtti(),
                                               REntity::PropertyLayer);
    RToleranceEntity::PropertyLinetype.generateId(RToleranceEntity::getRtti(),
                                                  REntity::PropertyLinetype);
    RToleranceEntity::PropertyLinetypeScale.generateId(
            RToleranceEntity::getRtti(), REntity::PropertyLinetypeScale);
    RToleranceEntity::PropertyLineweight.generateId(
            RToleranceEntity::getRtti(), REntity::PropertyLineweight);
    RToleranceEntity::PropertyColor.generateId(RToleranceEntity::getRtti(),
                                               REntity::PropertyColor);
    RToleranceEntity::PropertyDisplayedColor.generateId(
            RToleranceEntity::getRtti(), REntity::PropertyDisplayedColor);
    RToleranceEntity::PropertyDrawOrder.generateId(RToleranceEntity::getRtti(),
                                                   REntity::PropertyDrawOrder);

    RToleranceEntity::PropertyLocationX.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Location"),
            QT_TRANSLATE_NOOP("REntity", "X"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyLocationY.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Location"),
            QT_TRANSLATE_NOOP("REntity", "Y"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyLocationZ.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Location"),
            QT_TRANSLATE_NOOP("REntity", "Z"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyDirectionX.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Direction"),
            QT_TRANSLATE_NOOP("REntity", "X"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyDirectionY.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Direction"),
            QT_TRANSLATE_NOOP("REntity", "Y"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyDirectionZ.generateId(
            RToleranceEntity::getRtti(),
            QT_TRANSLATE_NOOP("REntity", "Direction"),
            QT_TRANSLATE_NOOP("REntity", "Z"), false,
            RPropertyAttributes::Geometry);
    RToleranceEntity::PropertyText.generateId(
            RToleranceEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Text"));
    RToleranceEntity::PropertyDimscale.generateId(RToleranceEntity::getRtti(),
                                                  RDimStyle::PropertyDimscale);
    if (RPluginLoader::hasPlugin("DWG"))
    {
        RToleranceEntity::PropertyDimtxt.generateId(RToleranceEntity::getRtti(),
                                                    RDimStyle::PropertyDimtxt);
    }
}
                                   ...
                                   RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMember(data.location.x, value,
                                    PropertyLocationX == propertyTypeId);
    ret = ret || RObject::setMember(data.location.y, value,
                                    PropertyLocationY == propertyTypeId);
    ret = ret || RObject::setMember(data.location.z, value,
                                    PropertyLocationZ == propertyTypeId);
    ret = ret || RObject::setMember(data.direction.x, value,
                                    PropertyDirectionX == propertyTypeId);
    ret = ret || RObject::setMember(data.direction.y, value,
                                    PropertyDirectionY == propertyTypeId);
    ret = ret || RObject::setMember(data.direction.z, value,
    ...
    ret = ret || RObject::setMember(getData().dimtxt, value,
                                    PropertyDimtxt == propertyTypeId);
...
RToleranceEntity::getProperty(RPropertyTypeId &propertyTypeId,
        ...
        if (v < 0.0)
        {
            ...
            if (doc != NULL)
            {
                ...
                if (!dimStyle.isNull())
                {
                    ...
                    else { v = dimStyle->getDouble(RS::DIMTXT); }
                }
            }
        }
...
void RToleranceEntity::exportEntity(RExporter &e, bool preview,
    ...
    for (int i = 0; i < labels.length(); i++)
    {
        ...
        if (textData.isSane())
        {
            RDimensionEntity::renderDimensionText(
                    e, doc, textData, data.isSelected(), forceSelected);
        }
