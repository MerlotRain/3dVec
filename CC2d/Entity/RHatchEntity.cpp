...
RHatchEntity *RHatchEntity::clone() const { return new RHatchEntity(*this); }
...
void RHatchEntity::init()
{
    ...
    RHatchEntity::PropertyWorkingSet.generateId(RHatchEntity::getRtti(),
                                                RObject::PropertyWorkingSet);
    RHatchEntity::PropertyType.generateId(RHatchEntity::getRtti(),
                                          REntity::PropertyType);
    RHatchEntity::PropertyBlock.generateId(RHatchEntity::getRtti(),
                                           REntity::PropertyBlock);
    RHatchEntity::PropertyLayer.generateId(RHatchEntity::getRtti(),
                                           REntity::PropertyLayer);
    RHatchEntity::PropertyLinetype.generateId(RHatchEntity::getRtti(),
                                              REntity::PropertyLinetype);
    RHatchEntity::PropertyLinetypeScale.generateId(
            RHatchEntity::getRtti(), REntity::PropertyLinetypeScale);
    RHatchEntity::PropertyLineweight.generateId(RHatchEntity::getRtti(),
                                                REntity::PropertyLineweight);
    RHatchEntity::PropertyColor.generateId(RHatchEntity::getRtti(),
                                           REntity::PropertyColor);
    RHatchEntity::PropertyDisplayedColor.generateId(
            RHatchEntity::getRtti(), REntity::PropertyDisplayedColor);
    RHatchEntity::PropertyDrawOrder.generateId(RHatchEntity::getRtti(),
                                               REntity::PropertyDrawOrder);

    RHatchEntity::PropertySolid.generateId(
            RHatchEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Solid"));
    RHatchEntity::PropertyTransparency.generateId(
            RHatchEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Alpha"));

    RHatchEntity::PropertyPatternName.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Pattern"),
            QT_TRANSLATE_NOOP("REntity", "Name"));
    RHatchEntity::PropertyEntityPattern.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Pattern"),
            QT_TRANSLATE_NOOP("REntity", "From Entity"));
    RHatchEntity::PropertyAngle.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Pattern"),
            QT_TRANSLATE_NOOP("REntity", "Angle"), false,
            RPropertyAttributes::Geometry);
    RHatchEntity::PropertyScaleFactor.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Pattern"),
            QT_TRANSLATE_NOOP("REntity", "Scale"), false,
            RPropertyAttributes::Geometry);

    RHatchEntity::PropertyOriginX.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Origin"),
            QT_TRANSLATE_NOOP("REntity", "X"));
    RHatchEntity::PropertyOriginY.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Origin"),
            QT_TRANSLATE_NOOP("REntity", "Y"));

    RHatchEntity::PropertyVertexNX.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Vertex"),
            QT_TRANSLATE_NOOP("REntity", "X"), false,
            RPropertyAttributes::Geometry);
    RHatchEntity::PropertyVertexNY.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Vertex"),
            QT_TRANSLATE_NOOP("REntity", "Y"), false,
            RPropertyAttributes::Geometry);
    RHatchEntity::PropertyVertexNZ.generateId(
            RHatchEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Vertex"),
                               ...
                               const QVariant &value, RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMember(data.solid, value,
                                    PropertySolid == propertyTypeId);
    ret = ret || RObject::setMember(data.transparency, value,
                                    PropertyTransparency == propertyTypeId);

    ret = ret || RObject::setMember(data.patternName, value,
                                    PropertyPatternName == propertyTypeId);
    ret = ret || RObject::setMember(data.scaleFactor, value,
                                    PropertyScaleFactor == propertyTypeId);
    ret = ret || RObject::setMember(data.angle, value,
                                    PropertyAngle == propertyTypeId);

    ret = ret || RObject::setMember(data.originPoint.x, value,
                                    PropertyOriginX == propertyTypeId);
    ret = ret || RObject::setMember(data.originPoint.y, value,
    ...
    ret = ret || setBoundaryVector(RObject::X, value,
                                   PropertyVertexNX == propertyTypeId);
    ret = ret || setBoundaryVector(RObject::Y, value,
                                   PropertyVertexNY == propertyTypeId);
    ret = ret || setBoundaryVector(RObject::Z, value,
...
RHatchEntity::getProperty(RPropertyTypeId &propertyTypeId, bool humanReadable,
                          bool noAttributes, bool showOnRequest)
{
    ...
    {
        return qMakePair(QVariant(data.transparency),
    ...
    if (propertyTypeId == PropertyPatternName)
    {
        return qMakePair(
                QVariant(name),
                RPropertyAttributes(RPropertyAttributes::Pattern | op));
    }
    else if (propertyTypeId == PropertyEntityPattern)
    {
        return qMakePair(QVariant(data.hasCustomPattern()
                                          ? QT_TRANSLATE_NOOP("REntity", "Yes")
                                          : QT_TRANSLATE_NOOP("REntity", "No")),
                         RPropertyAttributes(RPropertyAttributes::ReadOnly));
    }
    else if (propertyTypeId == PropertyScaleFactor)
    {
        return qMakePair(
                QVariant(data.scaleFactor),
                RPropertyAttributes(RPropertyAttributes::UnitLess | op));
    }
    else if (propertyTypeId == PropertyAngle)
    {
        return qMakePair(QVariant(data.angle),
                         RPropertyAttributes(RPropertyAttributes::Angle | op));
    }
        ...
        propertyTypeId == PropertyVertexNZ)
    {
        ...
        for (int i = 0; i < data.boundary.size(); ++i)
        {
            ...
            for (int k = 0; k < loop.size(); ++k)
            {
                ...
                if (!line.isNull()) { pointList.append(line->getStartPoint()); }

                QSharedPointer<RArc> arc = shape.dynamicCast<RArc>();
                if (!arc.isNull()) { pointList.append(arc->getStartPoint()); }

                QSharedPointer<RCircle> circle = shape.dynamicCast<RCircle>();
                if (!circle.isNull()) { pointList.append(circle->getCenter()); }

                QSharedPointer<REllipse> ellipseArc =
                        shape.dynamicCast<REllipse>();
                if (!ellipseArc.isNull())
                {
                    ...
                    else { pointList.append(ellipseArc->getStartPoint()); }
                }
...
void RHatchEntity::setViewportContext(const RViewportData &vp)
{
    ...
    // RVector offs = vp.getViewCenter() * vp.getScale() +
    //                vp.getViewTarget() * vp.getScale() -
    //                data.getOriginPoint() * vp.getScale();
