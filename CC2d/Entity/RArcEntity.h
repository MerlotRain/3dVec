 ...
 * \ingroup entity
 */
class CC2DCORE_EXPORT RArcEntity : public REntity
{
public:
...
public:
    ...
    static RS::EntityType getRtti() { return RS::EntityArc; }
    ...
    virtual RArcEntity *clone() const { return new RArcEntity(*this); }
    ...
    static QSharedPointer<REntity> scaleNonUniform(REntity &entity,
                                                   const RVector &scaleFactors,
                                                   const RVector &center);

    virtual RArcData &getData() { return data; }

    virtual const RArcData &getData() const { return data; }

    RVector getCenter() const { return data.getCenter(); }

    double getRadius() const { return data.getRadius(); }

    void setRadius(double radius) { data.setRadius(radius); }

    double getBulge() const { return data.getBulge(); }

    double getStartAngle() const { return data.getStartAngle(); }

    void setStartAngle(double angle) { data.setStartAngle(angle); }

    double getEndAngle() const { return data.getEndAngle(); }

    void setEndAngle(double angle) { data.setEndAngle(angle); }
    ...
    RVector getMiddlePoint() const { return data.getMiddlePoint(); }
