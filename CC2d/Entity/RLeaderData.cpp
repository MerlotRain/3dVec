...
RLeaderData::RLeaderData()
    : arrowHead(true), dimasz(-1.0), dimscale(-1.0),
      dimLeaderBlockId(REntity::INVALID_ID), splineShaped(false)
{
}
...
RLeaderData::RLeaderData(const RPolyline &polyline, bool arrowHead)
    : RPolyline(polyline), arrowHead(arrowHead), dimasz(-1.0), dimscale(-1.0),
      dimLeaderBlockId(REntity::INVALID_ID), splineShaped(false)
{
}
...
{
    if (countSegments() == 0) { return false; }

    QSharedPointer<RShape> firstSegment = getSegmentAt(0);
    if (firstSegment.isNull()) { return false; }

    if (firstSegment->getLength() < getDimasz() * 2) { return false; }

    return true;
}

QList<RRefPoint>
RLeaderData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
    Q_UNUSED(hint)
...
RLeaderData::getShapes(const RBox &queryBox, bool ignoreComplex, bool segment,
...
void RLeaderData::setDimLeaderBlockId(REntity::Id id) { dimLeaderBlockId = id; }
