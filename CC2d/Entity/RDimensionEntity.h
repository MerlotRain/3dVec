 ...
 * \ingroup entity
 */
class CC2DCORE_EXPORT RDimensionEntity : public REntity
{
...
public:
    ...
    static RS::EntityType getRtti() { return RS::EntityDimension; }
    ...
    void clearStyleOverrides() { getData().clearStyleOverrides(); }
    ...
    void setText(const QString &t) { getData().setText(t); }

    QString getText() const { return getData().getText(); }
    ...
    RTextData getTextData() const { return getData().getTextData(); }
    ...
    void setTextPosition(const RVector &p) { getData().setTextPosition(p); }

    RVector getTextPosition() const { return getData().getTextPosition(); }

    void setFontName(const QString &fn) { getData().setFontName(fn); }
    ...
    void setCustomTextPosition(bool on) { getData().setCustomTextPosition(on); }
    ...
    double getTextRotation() const { return getData().getTextRotation(); }

    double getLinearFactor() const { return getData().getLinearFactor(); }

    void setLinearFactor(double f) { getData().setLinearFactor(f); }

    double getDimscale() const { return getData().getDimscale(); }

    void setDimscale(double f) { getData().setDimscale(f); }

    QString getDimBlockName() const { return getData().getDimBlockName(); }
    ...
    bool isArrow1Flipped() const { return getData().isArrow1Flipped(); }
    void setArrow1Flipped(bool on) { getData().setArrow1Flipped(on); }
    bool isArrow2Flipped() const { return getData().isArrow2Flipped(); }
    void setArrow2Flipped(bool on) { getData().setArrow2Flipped(on); }

    bool isExtLineFix() const { return getData().isExtLineFix(); }
    void setExtLineFix(bool on) { getData().setExtLineFix(on); }

    double getExtLineFixLength() const
    {
        return getData().getExtLineFixLength();
    }
    void setExtLineFixLength(double v) { getData().setExtLineFixLength(v); }
