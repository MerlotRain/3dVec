 ...
 * \ingroup entity
 */
class CC2DCORE_EXPORT RDimRadialData : public RDimensionData
{
...
public:
    RDimRadialData();
    RDimRadialData(const RDimensionData &dimData, const RVector &chordPoint);

    virtual RS::EntityType getType() const { return RS::EntityDimRadial; }
    virtual bool isValid() const;
    virtual bool isSane() const;

    void setCenter(const RVector &p) { setDefinitionPoint(p); }

    RVector getCenter() const { return getDefinitionPoint(); }

    void setChordPoint(const RVector &p) { chordPoint = p; }

    RVector getChordPoint() const { return chordPoint; }
