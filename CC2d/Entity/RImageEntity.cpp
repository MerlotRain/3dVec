...
void RImageEntity::init()
{
    ...
    RImageEntity::PropertyWorkingSet.generateId(RImageEntity::getRtti(),
                                                RObject::PropertyWorkingSet);
    RImageEntity::PropertyType.generateId(RImageEntity::getRtti(),
                                          REntity::PropertyType);
    RImageEntity::PropertyBlock.generateId(RImageEntity::getRtti(),
                                           REntity::PropertyBlock);
    RImageEntity::PropertyLayer.generateId(RImageEntity::getRtti(),
                                           REntity::PropertyLayer);
    RImageEntity::PropertyLinetype.generateId(RImageEntity::getRtti(),
                                              REntity::PropertyLinetype);
    RImageEntity::PropertyLinetypeScale.generateId(
            RImageEntity::getRtti(), REntity::PropertyLinetypeScale);
    RImageEntity::PropertyLineweight.generateId(RImageEntity::getRtti(),
                                                REntity::PropertyLineweight);
    RImageEntity::PropertyColor.generateId(RImageEntity::getRtti(),
                                           REntity::PropertyColor);
    RImageEntity::PropertyDisplayedColor.generateId(
            RImageEntity::getRtti(), REntity::PropertyDisplayedColor);
    RImageEntity::PropertyDrawOrder.generateId(RImageEntity::getRtti(),
                                               REntity::PropertyDrawOrder);

    RImageEntity::PropertyFileName.generateId(
            RImageEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "File"));

    RImageEntity::PropertyInsertionPointX.generateId(
            RImageEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Position"),
            QT_TRANSLATE_NOOP("REntity", "X"), false,
            RPropertyAttributes::Geometry);
    RImageEntity::PropertyInsertionPointY.generateId(
            RImageEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Position"),
            QT_TRANSLATE_NOOP("REntity", "Y"), false,
            RPropertyAttributes::Geometry);
    RImageEntity::PropertyInsertionPointZ.generateId(
            RImageEntity::getRtti(), QT_TRANSLATE_NOOP("REntity", "Position"),
            QT_TRANSLATE_NOOP("REntity", "Z"), false,
            RPropertyAttributes::Geometry);

    RImageEntity::PropertyUX.generateId(RImageEntity::getRtti(),
                                        QT_TRANSLATE_NOOP("REntity", "U"),
                                        QT_TRANSLATE_NOOP("REntity", "X"),
                                        false, RPropertyAttributes::Geometry);
    RImageEntity::PropertyUY.generateId(RImageEntity::getRtti(),
                                        QT_TRANSLATE_NOOP("REntity", "U"),
                                        QT_TRANSLATE_NOOP("REntity", "Y"),
                                        false, RPropertyAttributes::Geometry);

    RImageEntity::PropertyVX.generateId(RImageEntity::getRtti(),
                                        QT_TRANSLATE_NOOP("REntity", "V"),
                                        QT_TRANSLATE_NOOP("REntity", "X"),
                                        false, RPropertyAttributes::Geometry);
    RImageEntity::PropertyVY.generateId(RImageEntity::getRtti(),
                                        QT_TRANSLATE_NOOP("REntity", "V"),
                                        QT_TRANSLATE_NOOP("REntity", "Y"),
                                        false, RPropertyAttributes::Geometry);

    RImageEntity::PropertyScaleFactorX.generateId(
            RImageEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Width Factor"), false,
            RPropertyAttributes::Geometry);
    RImageEntity::PropertyScaleFactorY.generateId(
            RImageEntity::getRtti(), "",
            QT_TRANSLATE_NOOP("REntity", "Height Factor"), false,
            RPropertyAttributes::Geometry);

    RImageEntity::PropertyWidth.generateId(
            RImageEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Width"),
            false, RPropertyAttributes::Geometry);
    RImageEntity::PropertyHeight.generateId(
            RImageEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Height"),
            false, RPropertyAttributes::Geometry);

    RImageEntity::PropertyAngle.generateId(
            RImageEntity::getRtti(), "", QT_TRANSLATE_NOOP("REntity", "Angle"),
            false, RPropertyAttributes::Geometry);

    RImageEntity::PropertyFade.generateId(RImageEntity::getRtti(), "",
                                          QT_TRANSLATE_NOOP("REntity", "Fade"));
}
                               ...
                               const QVariant &value, RTransaction *transaction)
{
    ...
    ret = ret || RObject::setMember(data.insertionPoint.x, value,
                                    PropertyInsertionPointX == propertyTypeId);
    ret = ret || RObject::setMember(data.insertionPoint.y, value,
                                    PropertyInsertionPointY == propertyTypeId);
    ret = ret || RObject::setMember(data.insertionPoint.z, value,
                                    PropertyInsertionPointZ == propertyTypeId);

    ret = ret || RObject::setMember(data.uVector.x, value,
                                    PropertyUX == propertyTypeId);
    ret = ret || RObject::setMember(data.uVector.y, value,
                                    PropertyUY == propertyTypeId);

    ret = ret || RObject::setMember(data.vVector.x, value,
                                    PropertyVX == propertyTypeId);
    ret = ret || RObject::setMember(data.vVector.y, value,
                                    PropertyVY == propertyTypeId);

    if (PropertyScaleFactorX == propertyTypeId)
    {
        double s = fabs(value.toDouble());
        if (s < RS::PointTolerance) { return false; }
    ...
    ret = ret ||
          RObject::setMember(data.fade, value, PropertyFade == propertyTypeId);
...
RImageEntity::getProperty(RPropertyTypeId &propertyTypeId, bool humanReadable,
                          bool noAttributes, bool showOnRequest)
{
    ...
    {
        return qMakePair(
                QVariant(data.uVector.getMagnitude() * data.getImage().width()),
                RPropertyAttributes(RPropertyAttributes::Redundant));
    }
    else if (propertyTypeId == PropertyHeight)
    {
        return qMakePair(QVariant(data.vVector.getMagnitude() *
                                  data.getImage().height()),
                         RPropertyAttributes(RPropertyAttributes::Redundant));
    }

    else if (propertyTypeId == PropertyAngle)
    {
        return qMakePair(QVariant(data.uVector.getAngle()),
    ...
    {
        return qMakePair(QVariant(data.fade),
                         RPropertyAttributes(RPropertyAttributes::Percentage));
    }
