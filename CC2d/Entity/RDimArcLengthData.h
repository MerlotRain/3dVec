...
public:
    RDimArcLengthData();
    RDimArcLengthData(const RDimensionData &dimData, const RVector &center,
                      const RVector &extensionLine1End,
                      const RVector &extensionLine2End);

    virtual RS::EntityType getType() const { return RS::EntityDimArcLength; }
    virtual bool isValid() const;
    virtual bool isSane() const;

    RVector getCenter() const { return center; }

    void setCenter(const RVector &p) { center = p; }

    double getRadius() const;

    void setExtensionLine1End(const RVector &p) { extensionLine1End = p; }

    RVector getExtensionLine1End() const { return extensionLine1End; }

    void setExtensionLine2End(const RVector &p) { extensionLine2End = p; }

    RVector getExtensionLine2End() const { return extensionLine2End; }

    void setDimArcPosition(const RVector &p) { setDefinitionPoint(p); }

    RVector getDimArcPosition() const { return getDefinitionPoint(); }

    void setArcSymbolType(int t) { arcSymbolType = t; }

    int getArcSymbolType() const { return arcSymbolType; }
    ...
    virtual bool getAngles(double &ang1, double &ang2, bool &reversed,
