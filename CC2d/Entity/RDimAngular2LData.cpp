...
RDimAngular2LData::RDimAngular2LData(RDocument *document,
...
RDimAngular2LData::RDimAngular2LData(const RDimensionData &dimData,
    ...
    : RDimAngularData(dimData), extensionLine1Start(extensionLine1Start),
      extensionLine1End(extensionLine1End),
      extensionLine2Start(extensionLine2Start), dimArcPosition(dimArcPosition)
{
}

bool RDimAngular2LData::isValid() const
{
    return RDimAngularData::isValid() && extensionLine1Start.isValid() &&
           extensionLine1End.isValid() && extensionLine2Start.isValid() &&
           dimArcPosition.isValid();
}

bool RDimAngular2LData::isSane() const
{
    return RDimAngularData::isSane() && extensionLine1Start.isSane() &&
           extensionLine1End.isSane() && extensionLine2Start.isSane() &&
           dimArcPosition.isSane();
}

QList<RRefPoint>
RDimAngular2LData::getReferencePoints(RS::ProjectionRenderingHint hint) const
{
...
bool RDimAngular2LData::scale(const RVector &scaleFactors,
                                  ...
                                  RVector &p1, RVector &p2) const
{
    ...
    {
        ang1 = RMath::getNormalizedAngle(
                extensionLine1End.getAngleTo(extensionLine1Start) + f1 * M_PI);
        if (f1 == 0) { p1 = extensionLine1Start; }
        else { p1 = extensionLine1End; }
        for (int f2 = 0; f2 <= 1 && !done; ++f2)
        {
            ang2 = RMath::getNormalizedAngle(
                    extensionLine2Start.getAngleTo(definitionPoint) +
                    f2 * M_PI);
            if (f2 == 0) { p2 = definitionPoint; }
            else { p2 = extensionLine2Start; }
            for (int t = 0; t <= 1 && !done; ++t)
            {
                ...
                if (RMath::isAngleBetween(ang, ang1, ang2, reversed) &&
                    angDiff <= M_PI)
                {
                    done = true;
                    break;
...
RVector RDimAngular2LData::getCenter() const
{
    ...
    if (vs.isEmpty()) { return RVector::invalid; }
