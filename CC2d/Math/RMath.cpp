...
{
#if defined Q_OS_WIN
    if (v == 0 || RMath::isNaN(v) || RMath::isInf(v)) { return v; }
    if (v >= 0.0) { return floor(v); }
    else { return ceil(v); }
#else
    return ::trunc(v);
#endif
...
{
    if (RMath::isNaN(v) || RMath::isInf(v)) { return false; }
    return true;
}
 ...
 */
QString RMath::angleToString(double a) { return QString("%1").arg(rad2deg(a)); }
...
bool RMath::isSameDirection(double dir1, double dir2, double tolerance)
{
    ...
    if (diff < tolerance || diff > 2 * M_PI - tolerance) { return true; }
    else { return false; }
}
...
QString RMath::toFractionString(double v, int maxDenominator)
{
    ...
    {
        if (numerator == 0) { return QString("%1").arg(number); }
        else
        {
 ...
 */
void RMath::toFraction(double v, int maxDenominator, int &number,
                       int &numerator, int &denominator)
{
    ...
    simplify(abs(mround((v - in) * maxDenominator)), maxDenominator, numerator,
             denominator);
}
 ...
 */
void RMath::simplify(int numerator, int denominator, int &numeratorRes,
                     int &denominatorRes)
{
...
bool RMath::containsFuzzy(const QList<double> &values, double v, double tol)
{
    ...
    {
        if (RMath::fuzzyCompare(v, values[i], tol)) { return true; }
    }
