 ...
 */
RLine::RLine() : startPoint(RVector::invalid), endPoint(RVector::invalid) {}
...
bool RLine::isValid() const { return startPoint.isSane() && endPoint.isSane(); }

double RLine::getLength() const { return startPoint.getDistanceTo(endPoint); }
...
void RLine::setLength(double l, bool fromStart)
{
    ...
    else { startPoint = endPoint - RVector::createPolar(l, getAngle()); }
}

double RLine::getAngle() const { return startPoint.getAngleTo(endPoint); }
...
double RLine::getDirection1() const { return startPoint.getAngleTo(endPoint); }

double RLine::getDirection2() const { return endPoint.getAngleTo(startPoint); }

RVector RLine::getStartPoint() const { return startPoint; }

void RLine::setStartPoint(const RVector &vector) { startPoint = vector; }

RVector RLine::getEndPoint() const { return endPoint; }

void RLine::setEndPoint(const RVector &vector) { endPoint = vector; }

RVector RLine::getMiddlePoint() const { return (startPoint + endPoint) / 2.0; }

RBox RLine::getBoundingBox() const
{
    return RBox(RVector::getMinimum(startPoint, endPoint),
                RVector::getMaximum(startPoint, endPoint));
}
...
QList<RVector> RLine::getCenterPoints() const { return getMiddlePoints(); }
                           ...
                           double strictRange) const
{
    ...
    if (ae.getMagnitude2D() < 1.0e-6) { return RVector::invalid; }
    ...
    if (limited && (b < 0 || b > 1.0))
    {
        ...
        if (ret.getMagnitude() < strictRange) { return ret; }
        else
        {
...
QSharedPointer<RShape> RLine::getTransformed(const QTransform &transform) const
{
    return QSharedPointer<RShape>(
            new RLine(startPoint.getTransformed2D(transform),
                      endPoint.getTransformed2D(transform)));
}
                             ...
                             const RVector &clickPoint)
{
    ...
    if (angleDifference < 0.0) { angleDifference *= -1.0; }
    if (angleDifference > M_PI)
    {
        angleDifference = 2 * M_PI - angleDifference;
    }

    if (angleDifference < M_PI / 2.0) { return RS::EndingStart; }
...
QList<QSharedPointer<RShape>> RLine::splitAt(const QList<RVector> &points) const
{
    ...
    QList<RVector> sortedPoints =
            RVector::getSortedByDistance(points, startPoint);
