 ...
 */
RArc::RArc()
    : center(RVector::invalid), radius(0.0), startAngle(0.0), endAngle(0.0),
      reversed(false)
{
}
...
void RArc::setZ(double z) { center.z = z; }
...
bool RArc::isValid() const { return center.isValid() && radius > 0.0; }
                            ...
                            double direction, double radius)
{
    ...
    if (fabs(diff - M_PI) < 1.0e-1) { arc.reversed = true; }
                              ...
                              bool secondTry)
{
    ...
    if (qAbs(radius1) < RS::PointTolerance ||
        qAbs(radius2) < RS::PointTolerance || !RMath::isNormal(radius1) ||
        !RMath::isNormal(radius2))
    {
        ...
        if (list.isEmpty()) { return QList<RArc>(); }

        for (int i = 0; i < list.length(); i++) { list[i].reverse(); }
    ...
    RArc arc1(center1, qAbs(radius1), center1.getAngleTo(startPoint),
              center1.getAngleTo(jointPoint));
    if (qAbs(RMath::getAngleDifference180(arc1.getDirection1(),
                                          startDirection)) > 0.1)
    {
        arc1.setReversed(true);
    }

    RArc arc2(center2, qAbs(radius2), center2.getAngleTo(jointPoint),
              center2.getAngleTo(endPoint));
    if (qAbs(RMath::getAngleDifference180(arc2.getDirection2() + M_PI,
                                          endDirection)) > 0.1)
    {
...
double RArc::getDirection1() const
{
    ...
    else { return RMath::getNormalizedAngle(startAngle - M_PI / 2.0); }
}

double RArc::getDirection2() const
{
    if (!reversed) { return RMath::getNormalizedAngle(endAngle - M_PI / 2.0); }
    else { return RMath::getNormalizedAngle(endAngle + M_PI / 2.0); }
}
...
RS::Side RArc::getSideOfPoint(const RVector &point) const
{
    if (reversed)
    {
        if (center.getDistanceTo(point) < radius) { return RS::RightHand; }
        else { return RS::LeftHand; }
    }
    else
    {
        if (center.getDistanceTo(point) < radius) { return RS::LeftHand; }
...
double RArc::getLength() const { return fabs(getAngleLength(false)) * radius; }

double RArc::getDiameter() const { return 2 * radius; }

void RArc::setDiameter(double d) { radius = d / 2.0; }
...
void RArc::setArea(double a)
{
    ...
    if (reversed) { endAngle = RMath::getNormalizedAngle(startAngle - sweep); }
    else { endAngle = RMath::getNormalizedAngle(startAngle + sweep); }
}
...
double RArc::getChordArea() const
{
    ...
    else if (sweep == M_PI) { sectorArea = 0.5 * (M_PI * radius * radius); }
    else
    {
...
double RArc::getSweep() const
{
    ...
    if (reversed)
    {
        ...
        else { ret = -(startAngle - endAngle); }
    }
    else
    {
        if (endAngle <= startAngle) { ret = endAngle + 2 * M_PI - startAngle; }
        else { ret = endAngle - startAngle; }
    }
...
RVector RArc::getCenter() const { return center; }

void RArc::setCenter(const RVector &vector) { center = vector; }

double RArc::getRadius() const { return radius; }

void RArc::setRadius(double r) { radius = r; }

double RArc::getStartAngle() const { return startAngle; }
...
double RArc::getEndAngle() const { return endAngle; }

void RArc::setEndAngle(double a) { endAngle = RMath::getNormalizedAngle(a); }
...
RVector RArc::getStartPoint() const { return getPointAtAngle(startAngle); }

RVector RArc::getEndPoint() const { return getPointAtAngle(endAngle); }
...
double RArc::getAngleAt(double distance, RS::From from) const
{
    ...
    if (points.length() != 1) { return RNANDOUBLE; }
    return center.getAngleTo(points[0]) + (reversed ? -M_PI / 2 : M_PI / 2);
}

bool RArc::isReversed() const { return reversed; }

void RArc::setReversed(bool r) { reversed = r; }
...
RBox RArc::getBoundingBox() const
{
    ...
    double a1 =
            RMath::getNormalizedAngle(!isReversed() ? startAngle : endAngle);
    double a2 =
            RMath::getNormalizedAngle(!isReversed() ? endAngle : startAngle);
    ...
    if (a1 > a2 - 1.0e-12) { maxX = qMax(center.x + radius, maxX); }
...
QList<RVector> RArc::getPointsWithDistanceToEnd(double distance, int from) const
{
    ...
    if (radius < RS::PointTolerance) { return ret; }
...
QSharedPointer<RShape> RArc::getTransformed(const QTransform &transform) const
{
    ...
    RArc *ret = new RArc(ct, ct.getDistanceTo(spt), ct.getAngleTo(spt),
                         ct.getAngleTo(ept), reversed);
...
RS::Ending RArc::getTrimEnd(const RVector &trimPoint, const RVector &clickPoint)
{
    ...
    {
        if (reversed) { return RS::EndingEnd; }
        else { return RS::EndingStart; }
    }
    else
    {
        if (reversed) { return RS::EndingStart; }
...
double RArc::getDistanceFromStart(const RVector &p) const
{
    ...
    if (reversed) { return RMath::getAngleDifference(ap, a1) * radius; }
    else { return RMath::getAngleDifference(a1, ap) * radius; }
}
...
RPolyline RArc::approximateWithLines(double segmentLength, double angle) const
{
    ...
    else
    {
        ...
        if (segmentLength > 0.0) { aStep = segmentLength / radius; }
        else
        {
                                        ...
                                        double angle) const
{
    ...
    else
    {
        ...
        {
            qWarning() << "RArc::approximateWithLinesTan: segmentLength to "
...
QList<QSharedPointer<RShape>> RArc::splitAt(const QList<RVector> &points) const
{
    ...
    QList<RVector> sortedPoints =
            RVector::getSortedByAngle(points, center, getStartAngle());
