 ...
 * \hasStreamOperator
 */
class CC2DCORE_EXPORT RTriangle : public RShape, public RExplodable
{
public:
    RTriangle();
    ...
    virtual RShape::Type getShapeType() const { return Triangle; }

    virtual RTriangle *clone() const { return new RTriangle(*this); }
    ...
    static RTriangle createArrow(const RVector &position, double direction,
                                 double size);
    ...
    bool isPointInTriangle(const RVector &p,
                           bool treatAsQuadrant = false) const;
    ...
    QSharedPointer<RShape> getTransformed(const QTransform &transform) const
    {
        return QSharedPointer<RShape>(
                new RTriangle(corner[0].getTransformed2D(transform),
     ...
     * \nonscriptable
     */
    RTriangle copy() const { return *this; }
#endif
