 ...
 * \copyable
 */
class CC2DCORE_EXPORT RLinetypePattern
{
...
public:
     ...
     * \nonscriptable
     */
    RLinetypePattern(bool metric, const QString &name,
                     const QString &description, int num...);

    RLinetypePattern(bool metric, const QString &name,
                     const QString &description, const QList<double> &dashes);
    ...
    bool isMetric() const { return metric; }
    void setMetric(bool on) { metric = on; }
    ...
    QList<QPainterPath> getShapeAt(int i) const;
    void updateShapes();
    ...
    QList<double> getSymmetries() const { return symmetries; }

    /**
     * \nonscriptable
     */
    double getPatternOffsetAt(double length, double symmetryPos,
                              double *gap = NULL, bool end = false);
...
private:
    ...
    QMap<int, QList<QPainterPath>> shapes;
