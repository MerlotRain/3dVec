...
bool RPatternLine::hasDots() const
{
    ...
    {
        if (RMath::fuzzyCompare(dashes[i], 0.0)) { return true; }
    }
    return false;
}
...
QList<RLine> RPatternLine::getLines(bool includeDashes) const
{
    ...
    else
    {
        for (int i = 0; i < dashes.length(); i++)
        {
            ...
            if (dash > 0) { ret.append(RLine(cursor, newCursor)); }
            else
            {
                if (includeDashes)
                {
...
{
    if (dashes.length() == 0) { return 10.0; }
    else
    {
        double ret = 0.0;
        for (int i = 0; i < dashes.length(); i++) { ret += qAbs(dashes[i]); }
        return ret;
    }
}
...
void RPatternLine::scale(double f)
{
    offset *= f;
    basePoint *= f;
    for (int i = 0; i < dashes.length(); i++) { dashes[i] *= f; }
}
