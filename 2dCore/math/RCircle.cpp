 ...
 */
RCircle::RCircle() : center(RVector::invalid), radius(0.0) {}
                                   ...
                                   const RVector &p3)
{
    ...
    if (ips.length() != 1) { return RCircle(); }
...
void RCircle::setZ(double z) { center.z = z; }
...
RVector RCircle::getCenter() const { return center; }

void RCircle::setCenter(const RVector &vector) { center = vector; }

double RCircle::getRadius() const { return radius; }

void RCircle::setRadius(double r) { radius = r; }

RBox RCircle::getBoundingBox() const
{
    return RBox(center - RVector(radius, radius),
                center + RVector(radius, radius));
}

double RCircle::getLength() const { return 2 * radius * M_PI; }

double RCircle::getDiameter() const { return 2 * radius; }

void RCircle::setDiameter(double d) { radius = d / 2.0; }

double RCircle::getCircumference() const { return radius * 2 * M_PI; }

void RCircle::setCircumference(double c) { radius = c / M_PI / 2.0; }

double RCircle::getArea() const { return radius * radius * M_PI; }

void RCircle::setArea(double a) { radius = sqrt(fabs(a) / M_PI); }
...
RCircle::getTransformed(const QTransform &transform) const
{
...
QList<RLine> RCircle::getTangents(const RVector &point) const
{
    ...
    if (ips.length() > 0)
    {
        ...
        if (ips.length() > 1) { ret.append(RLine(point, ips[1])); }
    }
...
RCircle::splitAt(const QList<RVector> &points) const
{
    ...
    QList<RVector> sortedPoints =
            RVector::getSortedByAngle(points, center, refAngle);
    ...
    for (int i = 0; i < sortedPoints.length() - 1; i++)
    {
        ...
        ret.append(QSharedPointer<RShape>(
                new RArc(center, radius, center.getAngleTo(sortedPoints[i]),
