...
class CC2DCORE_EXPORT RPolyline : public RShape, public RExplodable
{
public:
    RPolyline();
    ...
    virtual RShape::Type getShapeType() const { return Polyline; }

    virtual RPolyline *clone() const { return new RPolyline(*this); }
    ...
    virtual bool contains(const RVector &point, bool borderIsInside = false,
                          double tolerance = RS::PointTolerance) const;
    ...
    virtual double getDistanceFromStart(const RVector &p) const
    {
        ...
        if (res.isEmpty()) { return RMAXDOUBLE; }
        return res.first();
    }
    ...
    QList<RPolyline> getLeftOutline() const
    {
        ...
        QList<RPolyline> ret;
        for (int i = 0; i < lr.length(); i++) { ret.append(lr[i].first); }
        return ret;
    }
    QList<RPolyline> getRightOutline() const
    {
        QList<QPair<RPolyline, RPolyline>> lr = getLeftRightOutline();
        QList<RPolyline> ret;
        for (int i = 0; i < lr.length(); i++) { ret.append(lr[i].second); }
        return ret;
    }
    ...
    QList<RPolyline> morph(const RPolyline &target, int steps,
                           RS::Easing easing = RS::Linear, bool zLinear = true,
    ...
    static bool hasProxy() { return polylineProxy != NULL; }
    ...
    {
        if (polylineProxy != NULL) { delete polylineProxy; }
        polylineProxy = p;
    }
     ...
     * \nonscriptable
     */
    static RPolylineProxy *getPolylineProxy() { return polylineProxy; }
     ...
     * \nonscriptable
     */
    RPolyline copy() const { return *this; }
#endif
