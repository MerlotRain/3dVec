 ...
 */
REllipse::REllipse()
    : center(RVector::invalid), majorPoint(RVector::invalid), ratio(0.0),
                                   ...
                                   const RVector &centerHint)
{
    ...
    {
        ret = REllipse::getEllipseProxy()->createInscribed(p1, p2, p3, p4,
                                                           centerHint);
    }
...
{
    return center.isValid() && majorPoint.isValid() && !RMath::isNaN(ratio) &&
           !RMath::isNaN(startParam) && !RMath::isNaN(endParam);
}
...
{
    if (changeAngleOnly) { startParam = getParamTo(pos); }
    else
    {
...
{
    if (changeAngleOnly) { endParam = getParamTo(pos); }
    else
    {
...
RVector REllipse::getCenter() const { return center; }

void REllipse::setCenter(const RVector &vector) { center = vector; }

/**
 * \return Major point relative to the center point.
 */
RVector REllipse::getMajorPoint() const { return majorPoint; }
...
{
    if (fabs(ratio) < RS::PointTolerance) { return false; }
...
double REllipse::getRatio() const { return ratio; }
 ...
 */
double REllipse::getStartParam() const { return startParam; }

void REllipse::setStartParam(double a) { startParam = a; }

/**
 * \return End parameter: the end angle of the circular arc
 * resulting from transforming the ellipse into a circle.
 */
double REllipse::getEndParam() const { return endParam; }

void REllipse::setEndParam(double a) { endParam = a; }
...
{
    return RMath::getNormalizedAngle(center.getAngleTo(getStartPoint()) -
                                     getAngle());
}
...
{
    return RMath::getNormalizedAngle(center.getAngleTo(getEndPoint()) -
                                     getAngle());
}
...
double REllipse::angleToParam(double a) const
{
    ...
    else
    {
        ...
        if (r.length() != 1) { return RNANDOUBLE; }
...
double REllipse::getAngleLength(bool allowForZeroLength) const
{
    ...
    if (isReversed())
    {
        if (startParam < endParam) { ret = startParam + 2 * M_PI - endParam; }
        else { ret = startParam - endParam; }
    }
    else
    {
        if (endParam < startParam) { ret = endParam + 2 * M_PI - startParam; }
        else { ret = endParam - startParam; }
    }
...
{
    RVector p(center.x + cos(startParam) * getMajorRadius(),
...
{
    RVector p(center.x + cos(endParam) * getMajorRadius(),
 ...
 */
double REllipse::getMajorRadius() const { return majorPoint.getMagnitude(); }
 ...
 */
double REllipse::getAngle() const { return majorPoint.getAngle(); }
...
bool REllipse::isReversed() const { return reversed; }

void REllipse::setReversed(bool r) { reversed = r; }
...
RS::Side REllipse::getSideOfPoint(const RVector &point) const
{
    if (contains(point))
    {
        if (!reversed) { return RS::RightHand; }
        else { return RS::LeftHand; }
    }
    else
    {
        if (!reversed) { return RS::LeftHand; }
...
bool REllipse::scale(const RVector &scaleFactors, const RVector &c)
{
    ...
    {
        qWarning("REllipse::scale: scaling with different factors in X/Y not "
...
REllipse::getTransformed(const QTransform &transform) const
{
    ...
    QSharedPointer<REllipse> ret = QSharedPointer<REllipse>(
            new REllipse(ct, mpt - ct, ratio, 0.0, M_PI * 2, reversed));
...
QList<RLine> REllipse::getTangents(const RVector &point) const
{
    ...
    if (s1.isValid()) { ret.append(RLine(point, s1)); }

    if (s2.isValid()) { ret.append(RLine(point, s2)); }
                                                        ...
                                                        const RVector &position)
{
    ...
    if (position.isValid())
    {
        ...
        insides.append(center.getDistanceTo(position) <
...
REllipse::splitAt(const QList<RVector> &points) const
{
    ...
    QList<RVector> sortedPoints = RVector::getSortedByAngle(
            points, center, center.getAngleTo(startPoint));
