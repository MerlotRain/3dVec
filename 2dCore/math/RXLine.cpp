 ...
 */
RXLine::RXLine()
    : basePoint(RVector::invalid), directionVector(RVector::invalid)
{
}
...
{
    return RBox(RVector::getMinimum(basePoint, getSecondPoint()),
...
double RXLine::getAngle() const { return directionVector.getAngle(); }

void RXLine::setAngle(double a) { directionVector.setAngle(a); }
...
double RXLine::getDirection1() const { return directionVector.getAngle(); }
...
RVector RXLine::getStartPoint() const { return basePoint; }
...
RVector RXLine::getBasePoint() const { return basePoint; }

void RXLine::setBasePoint(const RVector &vector) { basePoint = vector; }

RVector RXLine::getSecondPoint() const { return basePoint + directionVector; }
...
RVector RXLine::getDirectionVector() const { return directionVector; }
...
QList<RVector> RXLine::getEndPoints() const { return QList<RVector>(); }

QList<RVector> RXLine::getMiddlePoints() const { return QList<RVector>(); }

QList<RVector> RXLine::getCenterPoints() const { return QList<RVector>(); }

QList<RVector> RXLine::getPointsWithDistanceToEnd(double distance,
                                                  int from) const
{
    Q_UNUSED(distance)
    Q_UNUSED(from)
...
RLine RXLine::getClippedLine(const RBox &box) const
{
    ...
    QList<RVector> ips =
            RShape::getIntersectionPointsLX(getLineShape(), pl, false);
    QList<RVector> sol;
...
RXLine::splitAt(const QList<RVector> &points) const
{
    if (points.length() == 0) { return RShape::splitAt(points); }

    QList<QSharedPointer<RShape>> ret;

    QList<RVector> sortedPoints = RVector::getSortedByDistance(
            points, basePoint - directionVector * 1e9);

    ret.append(QSharedPointer<RShape>(
            new RRay(sortedPoints[0], -directionVector)));
