 ...
 */
double RBox::getWidth() const { return qAbs(c2.x - c1.x); }

/**
 * \return Height of this box.
 */
double RBox::getHeight() const { return qAbs(c2.y - c1.y); }
...
{
    if (limited && !intersects(shape.getBoundingBox())) { return false; }

    QList<RLine> boxEdges = getLines2d();
    for (int i = 0; i < boxEdges.length(); i++)
    {
        if (boxEdges[i].intersectsWith(shape, limited)) { return true; }
    }
...
{
    for (int i = 0; i < others.length(); i++) { growToInclude(others[i]); }
}
